...
| 字段 | 默认值 | 用途 |
| --- | --- | --- |
| `cluster.critical_channel_capacity` | `1000` | 踢人、权限变更等高优先级事件 |
| `cluster.publish_channel_capacity` | `10000` | 普通 Redis publish 事件 |
...
`cluster.stream_max_length` 默认值：`10000`。它控制 Redis Stream 近似保留长度。太小会让断线节点还没追上时事件已被裁剪；太大会占用更多 Redis 内存。
