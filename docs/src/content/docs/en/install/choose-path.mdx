...
| --- | --- | --- | --- | --- |
| Single-node production Compose | Self-hosters, small teams, one server | Prebuilt image + `.env.postgres` + `.env.synctv` | Default production path | [Quick Start](../../install/quick-start/) |
...
| Source run | Developers, API debugging, code changes | Container dependencies + local `cargo run` | Not a deployment path | [Development Guide](../../develop/local-development/) |
...
| --- | --- | --- |
| Temporary evaluation | Local evaluation | Fast startup, but includes development secrets and must not face the public internet or run long term. |
...
| Multi-replica realtime | Helm or self-managed multi-replica | All replicas must share PostgreSQL, Redis, `redis.key_prefix`, and `server.cluster_secret`. |
