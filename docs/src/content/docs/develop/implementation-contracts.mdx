...
- `file_upload_sessions` 保存 database multipart 和 S3 multipart 的可恢复上传状态。
...
S3 默认返回客户端直传 S3 的 presigned URL。服务端代理上传/下载路径作为兼容能力保留。Database 后端的 URL 指向 SyncTV，因为字节存储在应用背后的数据库里。
