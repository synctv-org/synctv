...
3. 浏览器授权把用户跳转到第三方授权页面，第三方回调到客户端 URL 并带回 `code` 和 `state`。原生 Apple 授权由 iOS/macOS 的 Sign in with Apple 返回 `code` 和 `state`。原生授权响应中的 `nonce` 用作服务端 nonce 输入：客户端对它计算 SHA-256，并把小写十六进制摘要设置为 Apple 请求的 nonce；保留响应中的 `state`，在 exchange 时和 code 一起提交。
