...
This template covers all static configuration fields in the current version and shows representative dynamic objects. `oauth2.providers` and `media_providers.providers` are provider-specific, so their nested shapes cannot be exhaustively represented as static fields. See [Email and OAuth2](../email-oauth2/) and [Media Providers](../media-providers/) for details.
