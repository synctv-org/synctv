...
pub(crate) const THUMBNAIL_ROUTE: &str = "/api/providers/fnos/thumbnail";
const SIGNATURE_PROVIDER: &str = "fnos-thumbnail";

#[derive(Clone, Copy)]
pub(crate) struct FnosThumbnailScope<'a> {
    pub server_id: &'a str,
...
pub(crate) enum FnosThumbnailAccessError {
    Invalid,
    WrongUser,
}
...
pub(crate) fn fnos_thumbnail_url(
    server_id: &str,
...
pub(crate) fn fnos_own_thumbnail_url(server_id: &str, image_path: &str, width: u32) -> String {
    let query = url::form_urlencoded::Serializer::new(String::new())
        .append_pair("serverId", server_id)
        .append_pair("imagePath", image_path)
        .append_pair("width", &width.clamp(1, 1920).to_string())
...
pub(crate) fn sign_fnos_thumbnail_url(
    thumbnail_url: &str,
    room_id: &str,
    user_id: &str,
    signing_key: &ProxySigningKey,
...
pub(crate) fn verify_fnos_thumbnail_access(
    signing_key: &ProxySigningKey,
    auth_user_id: &str,
    raw_query: &str,
    scope: FnosThumbnailScope<'_>,
