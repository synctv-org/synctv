...
fn test_normal_object_still_parses() {
    ...
    assert!(result.is_ok(), "Normal nested object should parse successfully");
}
...
fn test_normal_large_object_parses() {
    ...
    assert!(result.is_ok(), "Object with 100 keys should parse successfully");
}
...
fn test_oversized_object_rejected() {
    ...
    assert!(result.is_err(), "Object with too many keys should be rejected");
...
fn test_ecma_array_depth_limit() {
    ...
    assert!(result.is_err(), "Deeply nested structure should be rejected");
}
...
fn test_exact_depth_boundary() {
    ...
    assert!(result.is_err(), "Object over depth limit (33) should be rejected");
}
