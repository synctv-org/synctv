...
字段默认值以源码默认配置为准。动态对象例如 `oauth2.providers` 和 `media_providers.providers` 的内部字段由具体 provider/backend 决定，因此这里只列出外层配置项。
...
| `oauth2` | 对象 | 见下方 | [邮件与 OAuth2](../../configuration/email-oauth2/) |
...
| `cache` | 对象 | 见下方 | [缓存与代理 slice cache](../../configuration/cache-and-proxy/) |
...
| 字段 | 类型/可选值 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `jwt.secret` | secret 字符串 | `""` | token 签名密钥 |
...
| `livestream.hls_shared_storage` | 布尔值 | `false` | HLS 是否使用多副本共享存储 |
| `livestream.hls_storage_path` | 路径 | `""` | HLS 文件存储路径 |
...
## `oauth2`、`webauthn`、`email`

| 字段 | 类型/可选值 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `oauth2.providers` | 动态对象 | `{}` | OAuth2/OIDC provider 配置 |
| `oauth2.redirect_scheme` | `http` 或 `https` | `https` | OAuth2 redirect scheme |
...
| 字段 | 类型/可选值 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `media_providers.providers` | 动态对象 | `{}` | Alist、Emby、Bilibili 等 provider 实例配置 |
| `media_providers.request_timeout_seconds` | 秒数 | `30` | Provider HTTP 请求总超时 |
| `media_providers.connect_timeout_seconds` | 秒数 | `10` | Provider HTTP 建连超时 |
