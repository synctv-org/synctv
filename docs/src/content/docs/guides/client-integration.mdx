...
<Steps>
1. 调用 `ListAvailableProviders` 获取可用 provider 实例、`signup_enabled` 和 `signup_need_review`。
2. 调用 `GetAuthorizationUrl` 获取第三方授权 URL 和 state。
3. 把用户跳转到第三方授权页面。
4. 第三方回调到客户端 URL，带回 `code` 和 `state`。
5. 客户端调用 `ExchangeAuthorizationCode`。
5. 如果响应包含 `registration_review_required=true`，展示待审核状态并保存 `registration_review_id`，不要把它当作已登录。
