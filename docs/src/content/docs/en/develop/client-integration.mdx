...
| Show synchronized playback | Read current playback state, fetch playback snapshot, observe `playback_state` and `playback_snapshot` | [Playback Model](../../concepts/playback-model/) |
...
| Handle URL expiry | Read `expires_at`, refresh playback snapshot before expiry, discard old URLs after media switch | [Playback and Proxy Model](../../use/playback-and-proxy/) |
...
Implement sign-in, room list, ticket creation, Realtime, and playback snapshots first. Then add Providers, chat, notifications, and WebRTC.
...
Finish the OPAQUE exchange with `/api/auth/opaque/login/finish`. A typical finish response contains `access_token`, `refresh_token`, `user`, and `mfa`. If `mfa.required=true`, complete MFA before treating the user as logged in. `/api/auth/login` is reserved for passwordless email-token login.
