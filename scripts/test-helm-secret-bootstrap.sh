...
missing_key="SYNCTV_SECURITY_EMAIL_OUTBOX_ENCRYPTION_KEY"
...
kubectl patch secret "$secret_name" \
  --namespace "$namespace" \
  --type=json \
  -p="[{\"op\":\"remove\",\"path\":\"/data/$missing_key\"}]" >/dev/null
...
reconciled="$(kubectl get secret "$secret_name" --namespace "$namespace" -o "jsonpath={.data.$missing_key}")"
