...
| Create attachment upload session | `POST /api/rooms/{room_id}/chat/attachments/upload-session` |
...
- Files and images use `CreateChatAttachmentUploadSession`, then the returned `ChatAttachment` goes into `SendChatMessage.attachments`. Requests must include `checksum_sha256`; the server uses global content-addressed objects for reuse and infers `kind` from MIME type. When `upload_required=true`, upload the object with `upload_url`, `upload_method`, and `upload_headers`. When `ownership_proof_required=true`, read the returned `ownership_proof_ranges` from the local file, compute the SHA-256 hex proof with `ownership_proof_nonce`, and write it into `attachment.metadata[ownership_proof_metadata_key]` before sending. The proof hash input order is `synctv-file-ownership-proof-v1`, one `0x00` byte, nonce UTF-8, then each range's `offset` as big-endian i64, `length` as big-endian i32, and range bytes.
