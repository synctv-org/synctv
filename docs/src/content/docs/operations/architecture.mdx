...
| 入口 | 默认端口或路径 | 作用 | 生产处理 |
| --- | --- | --- | --- |
| HTTP REST | `server.port=8080` | 客户端业务 API 和 OpenAPI UI | 通过反向代理或 Ingress 暴露 |
| 健康检查 | `health.port=8081` | 存活和就绪探针 | 仅对集群或本机开放 |
...
- 本地 HLS backend 走 publisher-node proxy；`shared_file` 会让当前节点从共享挂载读取 TS 分片，S3 提供对象存储分片。
...
直播路径和点播 proxy 路径不同。RTMP 是推流入口；HTTP-FLV 面向低延迟播放；HLS 通过 remuxer 生成 playlist/segment 并写入 `memory`、`file`、`shared_file` 或 `s3` backend。多副本时，publisher 所在节点通过共享 registry 暴露给其他节点；本地 backend 通过 HLS gRPC proxy 向 publisher 节点读取远端 playlist/segment，`shared_file` 的 TS 分片由当前节点从共享挂载读取。
...
<Diagram
  ...
  caption="Kubernetes 多副本部署应拆分 HTTP 与 gRPC 的 Service/Ingress，同时让所有 Pod 共享 PostgreSQL、Redis。本地 HLS backend 使用 publisher-node proxy；shared_file 会让当前节点从共享挂载读取 TS 分片，S3 则提供对象存储边界。"
/>
