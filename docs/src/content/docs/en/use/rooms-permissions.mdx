...
| `member` | Normal member; can chat, use WebRTC, add/edit/delete own media, and view playlist/member/chat data by default |
| `guest` | Guest, not a room member. Guests have no playlist, media, chat, or administration permissions by default. A room can only grant guest-safe capabilities such as member list, chat history, or WebRTC |
...
| Playlist and media | No | No | Guests cannot read playlists, browse media, or manage media |
| Chat and danmaku sending | No | No | `send_chat` is only for signed-in members |
...
- `guest_added_permissions` uses a dedicated guest ceiling. It cannot grant playlist, media, chat-write, or administration permissions.
...
| `edit_media_self` | Edit own media |
