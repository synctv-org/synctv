...
# Create .env file if it doesn't exist so subsequent CLI commands resolve the
# same development defaults as `synctv serve`.
...
if [ -f "$DEV_COMPOSE_FILE" ]; then
    ...
    # Run database migrations through the synctv CLI so the same embedded
    # migration path is exercised as production startup.
...
echo "  3. Start server:         cargo run --bin synctv -- serve"
echo ""
