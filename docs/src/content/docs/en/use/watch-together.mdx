...
The SyncTV server owns room playback state. A client fetches the current state and playback snapshot, then receives changes through Realtime.
...
| Switch media | Clients fetch a new playback snapshot |
