...
The “native app association” section covers WebAuthn/passkeys only. Native Sign in with Apple is an OAuth2 provider flow that uses an Apple App ID and a server-side client secret; see [Email and OAuth2](../email-oauth2/).
...
Each self-hosted server controls Android passkey association, allowing the official prebuilt app to add domains at runtime. Apple passkey RP domains are constrained by both the build signature and the server AASA document, so the domain set is determined at build time. This constraint does not apply to native Sign in with Apple; native Apple OAuth uses the signed app's Bundle ID and a server-side Apple client secret.
