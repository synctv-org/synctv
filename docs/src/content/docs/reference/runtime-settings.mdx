...
| `oauth2.allowedRedirectUrls` | JSON 字符串数组 | `[]` | 客户端可请求的 HTTPS 回调地址白名单；loopback 回调单独放行 |
...
Apple Services ID 只接受已登记的 HTTPS Return URL。Apple provider 的客户端回调必须与该 provider 的 `redirectUrl` 完全一致；macOS 和 iOS 客户端应通过 `ASWebAuthenticationSession` 接收这个 HTTPS 回调。
