...
3. 估算每分钟登录、刷新、Provider 浏览、播放快照刷新和列表读取量。
...
| `http_rate_limits.websocket_*` | WebSocket 建连限流 |
| `http_rate_limits.*` | HTTP 登录、API、媒体、管理和 streaming 限流 |
| `grpc_rate_limits.*` | gRPC API 和验证码限流 |
| `messaging_rate_limits.*` | 聊天和弹幕消息限流 |
...
<Tabs syncKey="capacity-tests">
  <TabItem label="API" icon="document">
    用真实登录、房间列表、成员列表、播放快照和 Provider 浏览路径压测。不要只压 `/health/ready`。
  </TabItem>
