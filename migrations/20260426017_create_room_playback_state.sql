...
CREATE TABLE IF NOT EXISTS room_playback_history (
    id BIGSERIAL,
    room_id BIGINT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE,
    ...
    CONSTRAINT room_playback_history_target_hash_sha256
        CHECK (target_hash ~ '^[0-9a-f]{64}$'),
    CONSTRAINT room_playback_history_target_object
        CHECK (target IS NULL OR jsonb_typeof(target) = 'object'),
    CONSTRAINT room_playback_history_media_same_room_fk
        FOREIGN KEY (media_id, room_id) REFERENCES media(id, room_id) ON DELETE CASCADE,
    CONSTRAINT room_playback_history_playlist_same_room_fk
        FOREIGN KEY (playlist_id, room_id) REFERENCES playlists(id, room_id) ON DELETE CASCADE
) PARTITION BY RANGE (created_at);

CREATE INDEX IF NOT EXISTS idx_room_playback_history_room_sequence
    ON room_playback_history(room_id, sequence DESC);
CREATE INDEX IF NOT EXISTS idx_room_playback_history_created_at
    ON room_playback_history(created_at, room_id, id);

CREATE TRIGGER update_room_playback_history_updated_at
    BEFORE UPDATE ON room_playback_history
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE IF NOT EXISTS room_playback_state (
    ...
    history_cursor_id BIGINT NULL,
    history_cursor_created_at TIMESTAMPTZ NULL,
    ...
    CONSTRAINT playback_playlist_same_room_fk
        ...
        REFERENCES playlists(id, room_id)
        ON DELETE RESTRICT,
    CONSTRAINT room_playback_state_history_same_room_fk
        FOREIGN KEY (room_id, history_cursor_id, history_cursor_created_at)
        REFERENCES room_playback_history(room_id, id, created_at)
        ON DELETE SET NULL (history_cursor_id, history_cursor_created_at),
    CONSTRAINT room_playback_state_history_cursor_complete
        CHECK (
            (history_cursor_id IS NULL AND history_cursor_created_at IS NULL)
            OR (history_cursor_id IS NOT NULL AND history_cursor_created_at IS NOT NULL)
        )
);
...
CREATE INDEX IF NOT EXISTS idx_room_playback_state_history_cursor_id ON room_playback_state(history_cursor_id);
