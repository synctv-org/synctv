...
/// Validate fixed-length internal entity IDs generated by the shared base62 nanoid generator.
pub fn validate_nanoid_id(id: &str, field_name: &'static str) -> ValidationResult<String> {
    ...
    if len > synctv_core::models::id::ID_LENGTH {
        return Err(ValidationError::TooLong {
            field: field_name,
            max: synctv_core::models::id::ID_LENGTH,
            actual: len,
        });
    }
    if len < synctv_core::models::id::ID_LENGTH {
        return Err(ValidationError::TooShort {
            field: field_name,
            min: synctv_core::models::id::ID_LENGTH,
            actual: len,
        });
    }
    if !synctv_common::id::is_valid_with_len(&sanitized, synctv_core::models::id::ID_LENGTH) {
...
mod tests {
    ...
    fn test_validate_nanoid_id() {
        ...
        assert!(validate_nanoid_id("AbC123xYz89", "user_id").is_err()); // Too short
        assert!(validate_nanoid_id("AbC123xYz8901", "user_id").is_err()); // Too long
