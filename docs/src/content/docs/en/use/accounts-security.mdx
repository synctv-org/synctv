...
| --- | --- | --- |
| Username and password (legacy mode) | Username and password | Compatibility or bootstrap deployments that keep legacy password mode |
...
<Steps>
1. Complete the first factor: OPAQUE password login, legacy password login, passkey, email sign-in, or OAuth2.
...
Public client local password login uses OPAQUE. `legacy_password` mode remains available for compatibility and bootstrap paths, but it does not mean public clients use plaintext password login or password as a second factor.
...
When reporting a problem, include time, username, sign-in method, error text, and `request_id`. Do not send passwords, tokens, cookies, OAuth2 codes, or verification codes.
