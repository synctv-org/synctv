...
| 目标 | 动作 | 注意 |
| --- | --- | --- |
| 临时禁言 | 在成员权限里移除 `send_chat_messages` | 比修改全房间聊天设置影响更小 |
| 给某人控制播放 | 给成员增加 `control_playback_state` | 如果还要切媒体，加 `navigate_playback` |
...
6. 用 [Realtime API](../../develop/realtime-api/) 订阅播放状态、播放信息、房间设置和成员列表。
7. 按 [错误参考](../../reference/errors/) 处理 `status`、`code`、`requestId` 和 `Retry-After`。
</Steps>
