...
`permissions.guest_default` has a dedicated guest ceiling. It rejects `view_playlist`, `send_chat`, media, playback-control, and administration permissions. Valid example:
...
User-level 2FA, notification preferences, and provider defaults are user preferences, not runtime settings. See [Rooms, Permissions, and Preferences](../../use/rooms-permissions/).
