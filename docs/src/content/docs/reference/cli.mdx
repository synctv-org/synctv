...
如果你要理解管理动作的业务语义，先看 [管理 SyncTV](../../admin/)。如果你要修改房间角色、权限名称、房间设置或用户偏好，先看 [房间、权限与用户偏好](../../use/rooms-permissions/)。
...
`--request-json` 接受标准 `UpdateSettingsRequest` ProtoJSON。`updateMask` 是逗号分隔的 lowerCamel FieldMask 字符串。
