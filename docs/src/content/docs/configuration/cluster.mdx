...
<Diagram
  ...
  caption="集群模式下，业务持久状态进入 PostgreSQL，跨节点运行时状态进入 Redis。HLS 可以使用本地 backend 加 publisher-node proxy，也可以使用共享文件系统或 OSS 让各节点直接读取分片。"
/>
...
<Aside type="caution">
集群模式不能修复底层依赖不一致。多个副本必须共享同一个 PostgreSQL、Redis 和 `server.cluster_secret`。HLS 本地 backend 可以通过 publisher-node proxy 工作，但 publisher 节点不可达、重启或网络分区时，远端节点读取分片会受影响；共享文件系统或 OSS 可以降低这类跨节点依赖。
</Aside>
...
- HLS 模型已明确：小规模使用本地 backend + publisher-node proxy，生产高流量使用 `file + hls_shared_storage=true + RWX/PVC` 或 `oss`。
