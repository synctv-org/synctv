...
| `permissions.guestDefaultPermissions` | Permission-name array | `[]` | Global default permissions for room guest; only `view_members`, `view_chat_history`, and `use_webrtc` are accepted |
...
`permissions.guestDefaultPermissions` has a dedicated guest ceiling. It rejects `view_media`, `send_chat_messages`, media-resource, playback-control, and administration permissions. Valid example:
