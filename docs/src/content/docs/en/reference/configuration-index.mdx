...
Defaults in this page are source-level defaults. `synctv config show --output yaml` prints the effective configuration after defaults, config file, environment variables, CLI overrides, path resolution, and timezone fallback are applied. OAuth2 provider instances are runtime settings; see [Runtime Settings](../runtime-settings/#oauth2). Remote media provider instances are persisted through the management API/CLI and are not static config-file fields.

## Full Configuration Example

The full YAML template is kept on a dedicated page so this index stays scannable:

- [Full Configuration Example](../../configuration/full-example/): minimal production skeleton, full YAML template covering all static fields, and URL/split configuration examples.
- `synctv config show --output yaml`: prints the current binary's complete effective configuration with secrets redacted.

## Make Configuration Decisions First

If you are not looking up a specific field, use this table to decide what actually needs changing. Most fields can keep their defaults. Production launch should prioritize secrets, database, entrypoints, security boundaries, and backups.
...
| `cluster.*` / `server.cluster_secret` | Multi-replica cluster | Cluster enabled without Redis or cluster secret | Restart required | All replicas share PostgreSQL, Redis, and secrets |
...
If you use production Compose, maintain `.env.postgres` and `.env.synctv` first. You usually do not need to hand-write a full YAML file. Before launch, confirm:
...
| Redis | Compose Redis service and key prefix | `redis.*` |
| Runtime files | Compose `/data` volume | `data_dir` |

Use the full field index below when you leave the Compose path for bare-metal, Kubernetes, secret files, or custom YAML.
