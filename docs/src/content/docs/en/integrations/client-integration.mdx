...
3. Browser authorization redirects the user to the provider, which returns `code` and `state` to the client callback. Native Apple authorization uses Sign in with Apple on iOS/macOS and returns `code` and `state` through the platform API.
...
Native Apple login uses the Apple App ID Bundle ID as `nativeClientId`; the server exchanges the authorization code with `nativeClientSecret`. The official client Bundle ID is `org.synctv.app`. A self-hosted distribution normally needs its own Apple Developer Team, Bundle ID, signing, and server-side secret. These settings are separate from the Apple Services ID and HTTPS `redirectUrl` used by browser authorization.
...
Use `GET /api/oauth2/{provider}/bind?redirectUrl=<callback>` for browser authorization when binding a provider to an existing authenticated account. Use `GET /api/oauth2/{provider}/bind?native=true` and omit `redirectUrl` for native Apple authorization. The bind authorization request and the later `POST /api/oauth2/{provider}/exchange` call must both include the current user's access token; the exchange token user must match the user captured in the OAuth2 state. OAuth2 login is not a local 2FA first or second factor, but users with 2FA enabled can still log in through OAuth2.
