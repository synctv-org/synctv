...
6. Today, cross-node HLS reads use the publisher-node HLS gRPC proxy as the main path. A shared `file` or `oss` backend gives the stream a more stable storage boundary, but it does not make non-publisher nodes bypass the publisher and read directly.
</Steps>

<Aside type="note">
Multi-replica HLS uses the publisher registry to locate the publishing node, then reads remote playlists and segments through the HLS gRPC proxy. Shared filesystems or OSS make segment storage more durable and easier to reason about across restarts, but capacity planning must still include publisher-node proxy traffic.
</Aside>
...
With local file-backed HLS in multi-replica deployments, non-publisher nodes use the HLS gRPC proxy to read from the publisher node. Shared storage keeps segments from being tied to one pod's local disk and gives cleaner restart and recovery boundaries, but the request path still goes through publisher-node proxying:
