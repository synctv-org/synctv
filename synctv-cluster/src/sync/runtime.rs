    ...
    match profile.state_mode() {
        SharedStateMode::SharedRequired => {
            let shared_runtime = profile.shared_runtime().ok_or_else(|| {
                Error::Configuration(
                    "cluster runtime requires shared realtime connection state".to_string(),
                )
            })?;
...
pub fn build_room_message_runtime(
    ...
    match profile.state_mode() {
        SharedStateMode::SharedRequired => {
            let shared_runtime = profile.shared_runtime().ok_or_else(|| {
                Error::Configuration(
                    "cluster runtime requires shared realtime message state".to_string(),
                )
            })?;
...
pub trait RoomMessageRuntime: Send + Sync {
    ...
    ) -> Result<mpsc::Receiver<ClusterEvent>>;
...
mod tests {
    ...
    fn test_build_room_message_runtime_requires_shared_runtime_when_cluster_state_is_required() {
        ...
        assert!(
            error
                .to_string()
                .contains("cluster runtime requires shared realtime message state"),
            "unexpected error: {error}"
        );
    }
