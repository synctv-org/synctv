...
fn test_notification_message_encode_decode() {
    ...
    let notification = ServerMessage {
        message: Some(Message::Notification(synctv_api::proto::client::UserNotification {
            ...
            data: r#"{"room_id":"room123","room_name":"Test Room","inviter_name":"Alice"}"#.to_string(),
    ...
    let decoded = ProtoCodec::decode_server_message(&encoded)
        .expect("Failed to decode notification message");
    ...
    match decoded.message {
        Some(Message::Notification(notif)) => {
            ...
            assert_eq!(notif.timestamp, 1704067200000);
        }
...
fn test_error_messages_still_work() {
    ...
    let encoded = ProtoCodec::encode_server_message(&error)
