...
| TOTP 加密 key | `security.totp_encryption_key` 是独立的 64 个十六进制字符，已备份。 |
| 邮件队列加密 key | `security.email_outbox_encryption_key` 是 64 个十六进制字符，所有副本一致并已备份。 |
| 短期安全域 key | 代理签名、媒体 swarm、Provider 会话、登录发现、WebAuthn 枚举和文件 token 使用六个不同值。 |
...
<Aside type="caution">
OPAQUE、Provider credential 和 email outbox 使用各自的长期 secret。备份需要覆盖全部 secret 与 PostgreSQL。
</Aside>
...
| 检查项 | 验收标准 |
| --- | --- |
| HTTP 入口 | [Nginx 反向代理](../nginx/)或 Ingress 支持 WebSocket Upgrade；Nginx 代理缓存和响应缓冲已关闭。 |
...
| OAuth2 | 浏览器授权客户端请求的 `redirectUrl` 已加入 `oauth2.allowedRedirectUrls`，并与第三方平台注册值一致；Apple provider 已配置 Web Services ID 和与签名 App Bundle ID 一致的 `nativeClientId`，并生成对应的服务端 secrets。 |
| Provider 凭据 | 凭据通过 secret 或受控管理流程写入，不出现在仓库。 |
| Slice cache 文件后端 | 缓存目录有容量规划和清理策略。 |
| 直播 HLS | 单机已确认本地存储；多副本已选择 publisher-node proxy、`shared_file` 或 S3。 |
