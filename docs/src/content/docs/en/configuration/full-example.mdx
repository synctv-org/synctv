...
If you followed [Quick Start](../../guides/quick-start/) with production Compose, you usually do not need to start with `synctv.yaml`. Your default entrypoint is `.env.postgres` and `.env.synctv`.
...
# init-compose-env.sh generates these long-lived secrets. Back them up and reuse the same files.
SYNCTV_JWT_SECRET=...
...
Use the YAML templates below when you need bare-metal, Kubernetes, custom secret files, or a full YAML configuration.
...
<Aside type="note">
This YAML skeleton is better suited for bare-metal, Kubernetes, or custom secret-file deployments. Production Compose users should maintain `.env.postgres` and `.env.synctv` first.
</Aside>
...
This template covers all static configuration fields in the current version. Provider fields only configure local built-in provider adapters in this process; remote provider instances are persisted through the management API/CLI. See [Media Providers](../media-providers/) for details.
