...
description: SyncTV playback snapshots, playback URLs, Provider headers, proxy modes, Range, slice cache, WebRTC, and livestream boundaries.
---
...
<Steps>
1. The client requests the current room playback or a playback snapshot for a media item.
2. SyncTV uses room state, media, user, Provider credentials, and client profile to build `PlaybackSnapshot`.
...
| URL stops working later | `expires_at` | Observe or fetch a new playback snapshot |
