...
| `chat_event` or `resource_changed.chat_event` | Chat message event after explicit subscription |
...
Chat message events use their own cursor: in bidirectional streams, send `observe_resource.chat_events.after_event_id` to replay events after a known event; HTTP SSE `GET /api/rooms/<room_id>/watch/chat-events` accepts the `after_event_id` query parameter and reads the browser's `Last-Event-ID` header on reconnect. The server sets the SSE `id:` field, so clients can persist that value for recovery.
