...
// =============================================================================
// Timeout Tests (marked #[ignore] - require Docker/testcontainers)
// =============================================================================
...
mod timeout_tests {
    ...
    /// Helper to create a mock RTMP server
    ...
    #[tokio::test]
    #[ignore = "Requires network and may be slow - run with --ignored flag"]
    async fn test_client_handshake_timeout_on_slow_server() {
        let server = MockRtmpServer::bind().await;
        let addr = server.addr();

        // Spawn server that accepts but delays response
    ...
    #[tokio::test]
    #[ignore = "Requires network and may be slow - run with --ignored flag"]
    async fn test_server_handshake_timeout_on_slow_client() {
        // Test that server handles client disappearing mid-handshake
        // This would require implementing timeout in the server handshake
        let server = MockRtmpServer::bind().await;
        let addr = server.addr();

        // Spawn server that expects handshake
        let server_handle = tokio::spawn(async move {
