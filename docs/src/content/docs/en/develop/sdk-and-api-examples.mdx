...
curl -sS "$BASE_URL/api/auth/opaque/login/start" \
  ...
  -d '{"username":"root","credential_request":[/* bytes from your OPAQUE client */]}'
```

Finish the OPAQUE exchange with `/api/auth/opaque/login/finish`. If the finish response has `mfa.required=true`, complete MFA before treating the user as logged in. `/api/auth/login` is reserved for passwordless email-token login.
...
`AuthService/Login` is reserved for passwordless email-token login. Legacy password mode is retained for compatibility/bootstrap paths and is not exposed by this public request.
...
curl -sS "$BASE_URL/api/providers/alist/list" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"server_id":"<server_id-from-login-or-binds>","path":"/","provider_instance_name":"alist-main"}'
```

When adding media, the `source_provider`, `provider_instance_name`, and `source_config` combination is defined by the Provider. Provider credentials should not be placed directly in `source_config`; they are resolved by the server-side Provider credential layer.
