...
impl HuyaPlaybackProviderService for HuyaPlaybackProviderGrpcService {
    ...
    async fn get_resource(
        ...
        execute_playback_provider_stream(state.clone(), metadata, move |request_control| {
            ...
            async move {
                crate::impls::playback_provider::huya::get_huya_resource(
    ...
    async fn get_segment(
        ...
        execute_playback_provider_stream(state.clone(), metadata, move |request_control| {
            let state = state.clone();
            async move {
                crate::impls::playback_provider::huya::get_huya_segment(
                    huya_deps(&state, Some(&request_control)),
                    req,
                )
                .await
    ...
    async fn watch_danmaku(
        ...
        execute_playback_provider_stream(state.clone(), metadata, move |request_control| {
            let state = state.clone();
            async move {
                crate::impls::playback_provider::huya::watch_huya_danmaku(
                    huya_deps(&state, Some(&request_control)),
                    req,
                )
                .await
...
) -> crate::impls::playback_provider::huya::HuyaPlaybackProviderDeps<'a> {
    crate::impls::playback_provider::huya::HuyaPlaybackProviderDeps {
