...
<Tabs syncKey="client-http-flow">
  <TabItem label="登录" icon="approve-check-circle">
...
典型成功响应会包含 `access_token`、`refresh_token`、`user` 和 `mfa`。如果 `mfa.required=true`，先完成 MFA，不要把这次响应当作已登录。
...
生产环境必须用 TLS。登录、注册、改密码、密码找回、MFA password 都必须走 HTTPS 或受信任的加密隧道。
