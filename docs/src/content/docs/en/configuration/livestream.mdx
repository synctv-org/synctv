...
description: RTMP, RTSP, HLS, HTTP-FLV, pull retry behavior, GOP cache, and livestream storage.
---
...
`livestream` controls RTMP publishing, external RTMP/RTSP/HTTP-FLV pulls, HLS segments, HTTP-FLV connections, livestream memory cache, and livestream file storage.
...
<Diagram
  ...
  alt="SyncTV livestream pipeline showing RTMP ingest entering StreamHub, splitting into HTTP-FLV and HLS remuxer paths, and HLS playlist/segment routes reading from memory, file, or S3 storage."
...
<Aside type="note">
RTMP is the publishing entry point. Playback uses SyncTV-generated HLS or HTTP-FLV links. Both outputs follow the current live edge, and room synchronization uses live playback state.
</Aside>
...
The RTMP provider media source config accepts the same `mode` field:
