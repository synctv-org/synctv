...
> {
    ...
    let download = service
        ...
        .await
        .map_err(map_api_error)?;
    let stream = synctv_api_common::impls::client::file_download::room_cover_chunk_stream(download)
...
> {
    ...
    let download = service
        ...
        .map_err(map_api_error)?;
    let stream =
        synctv_api_common::impls::client::file_download::media_cover_chunk_stream(download)
            .map(|result| result.map_err(map_api_error));
    Ok(Response::new(Box::pin(stream)))
}
...
> {
    ...
    let download = service
        ...
        .map_err(map_api_error)?;
    let stream =
        synctv_api_common::impls::client::file_download::media_thumbnail_chunk_stream(download)
            .map(|result| result.map_err(map_api_error));
    Ok(Response::new(Box::pin(stream)))
}
...
> {
    ...
    let download = service
        ...
        .map_err(map_api_error)?;
    let stream =
        synctv_api_common::impls::client::file_download::playlist_cover_chunk_stream(download)
            .map(|result| result.map_err(map_api_error));
    Ok(Response::new(Box::pin(stream)))
}
