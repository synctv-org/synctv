...
### `livestream.hls_storage_backend`
...
- `shared_file`，并且 `hls_storage_path` 是所有副本可读写的共享文件系统。`.ts` 请求会由当前节点直接从共享路径读取。
...
当 `hls_storage_backend=oss` 时，SyncTV 使用 S3 兼容对象存储保存 HLS 分片。
