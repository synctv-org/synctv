---
title: 直播与 HLS/FLV
description: RTMP、RTSP、HLS、HTTP-FLV、拉流重试、GOP 缓存和直播存储配置。
---
...
`livestream` 控制 RTMP 推流、RTMP/RTSP/HTTP-FLV 外部拉流、HLS 分片、HTTP-FLV 连接、直播内存缓存和直播文件存储。
...
<Diagram
  ...
  alt="SyncTV 直播流水线图，展示 RTMP 推流进入 StreamHub，分发到 HTTP-FLV 与 HLS remuxer，HLS 写入 memory/file/S3 存储后由 playlist 和 segment 路由读取。"
...
<Aside type="note">
RTMP 是推流入口，播放端使用 SyncTV 生成的 HLS 或 HTTP-FLV 链接。两种输出都跟随当前直播边缘，房间同步以实时播放状态为准。
</Aside>
...
RTMP provider 的媒体 source config 也支持相同的 `mode` 字段：
