...
fn test_cluster_auth_interceptor_correct_secret() {
    use synctv_api::ClusterAuthInterceptor;
...
fn test_logout_blacklist_failure_maps_to_grpc_internal() {
    let api_err = synctv_api::ApiError::Internal("Blacklist store unavailable".to_string());
    let proto_err = api_err.to_proto_error();
    assert_eq!(proto_err.code, synctv_api::ErrorKind::Internal.to_code());
