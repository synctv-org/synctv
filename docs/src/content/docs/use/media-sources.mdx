...
内置 Provider 类型包括 `alist`、`bilibili`、`emby`、`direct_url`、`rtmp` 和 `live_proxy`。远程 Provider instance 用数据库持久化的连接信息接入另一组 Provider 服务，不是 `media_providers` YAML 对象。
...
<Aside type="caution">
不要把 Provider token、Cookie、密码或 API key 放进客户端日志、URL query 或媒体 `source_config`。凭据应由服务端 Provider 凭据管理层解析。
</Aside>
...
<Tabs syncKey="provider-recipes">
  ...
  <TabItem label="RTMP/直播" icon="youtube">
    ...
    **重点**：
    ...
    - 房间成员需要 `live_control` 权限才能管理直播和创建推流 key。
...
- 禁止把 Cookie、token、API key 写进 `source_config`、URL query、日志或截图。
