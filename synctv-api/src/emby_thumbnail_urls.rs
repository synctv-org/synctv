...
pub(crate) const THUMBNAIL_ROUTE_PREFIX: &str = "/api/providers/emby/thumbnail/";
pub(crate) const THUMBNAIL_SIGNATURE_PROVIDER: &str = "emby-thumbnail";

#[derive(Clone, Copy)]
pub(crate) struct ThumbnailSignatureScope<'a> {
    pub item_id: &'a str,
    pub server_id: &'a str,
...
pub(crate) enum ThumbnailSignatureAccessError {
    Invalid,
    WrongUser,
}

pub(crate) fn clamp_thumbnail_dimension(value: Option<u32>, default: u32) -> u32 {
    value
...
pub(crate) fn emby_thumbnail_url(
    server_id: &str,
...
pub(crate) fn build_signed_thumbnail_query(
...
pub(crate) fn thumbnail_signature_present(raw_query: &str) -> bool {
    url::form_urlencoded::parse(raw_query.as_bytes()).any(|(key, _)| key.as_ref() == "sig")
}

pub(crate) fn thumbnail_signature_query(raw_query: &str) -> String {
...
pub(crate) fn verify_signed_thumbnail_access(
    signing_key: &ProxySigningKey,
    auth_user_id: &str,
    raw_query: &str,
    scope: ThumbnailSignatureScope<'_>,
