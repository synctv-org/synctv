...
| `file_storage.chat_images_backend` | backend 名称或空 | `""` | 聊天图片后端；空值继承 `default_backend` |
| `file_storage.user_avatars_backend` | backend 名称或空 | `""` | 用户头像后端；空值继承 `default_backend` |
| `file_storage.video_covers_backend` | backend 名称或空 | `""` | 视频封面后端；空值继承 `default_backend` |
...
| `file_storage.backends.<name>.database.compression` | `none`、`lz4` 或 `zstd` | `zstd` | `type: "database"` 后端写入 PostgreSQL `file_blobs` 的压缩算法 |
| `file_storage.backends.<name>.s3.*` | S3 配置 | S3 默认字段 | `type: "s3"` 后端的 S3-compatible 配置 |

客户端在上传会话请求中提供 `checksum_sha256`。选中的后端按 `(storage_backend, checksum_sha256, size_bytes)` 存储内容寻址对象，重复文件会返回 `upload_required=false` 和 ownership proof challenge。读取和清理使用对象记录里的 `storage_backend`，业务数据可以在配置切换后混合多个后端。
...
file_storage:
  ...
  chat_images_backend: "database"
  user_avatars_backend: "database"
  video_covers_backend: "database"
...
file_storage:
  ...
  chat_images_backend: "s3_public"
  user_avatars_backend: "s3_public"
  video_covers_backend: "s3_public"
...
| `grpc_rate_limits.websocket_max_requests` | 整数 | `10` | gRPC MessageStream 建连窗口内最大请求数 |
| `grpc_rate_limits.websocket_window_seconds` | 秒数 | `60` | gRPC MessageStream 建连限流窗口 |
