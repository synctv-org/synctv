...
| 视频首帧很慢或实时流延迟持续增加 | Nginx 正在缓存或缓冲后端响应 | 按 [Nginx 反向代理](../../install/nginx/)关闭 `proxy_cache` 和 `proxy_buffering` |
| Range seek 或代理播放失败 | Provider header 不完整或上游不支持 Range | 检查 Provider header 和上游 `Accept-Ranges` |
| 多副本 HLS 间歇 404 | publisher 节点不可达或 HLS 存储模型不一致 | 使用 publisher-node proxy、`shared_file` 或 S3 |
...
| 媒体 | Provider header、Range、proxy、slice cache、HLS backend | [播放与代理](../../integrations/playback-and-proxy/) |
