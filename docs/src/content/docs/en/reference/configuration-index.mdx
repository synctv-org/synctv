...
Defaults in this page are source-level defaults. `synctv config show --output yaml` prints the effective configuration after defaults, config file, environment variables, CLI overrides, path resolution, and timezone fallback are applied. Dynamic objects such as `oauth2.providers` and `media_providers.providers` are provider-specific; this page lists their outer static fields.
...
| `oauth2` | object | see below | [Email and OAuth2](../../configuration/email-oauth2/) |
...
| `cache` | object | see below | [Cache and Proxy Slice Cache](../../configuration/cache-and-proxy/) |
...
| `livestream.hls_shared_storage` | boolean | `false` | Whether HLS storage is shared across replicas |
| `livestream.hls_storage_path` | path | `""` | HLS file storage path |
...
## `oauth2`, `webauthn`, and `email`

| Field | Type / values | Default | Meaning |
| --- | --- | --- | --- |
| `oauth2.providers` | dynamic object | `{}` | OAuth2/OIDC provider configuration |
| `oauth2.redirect_scheme` | `http` or `https` | `https` | OAuth2 redirect URL scheme |
...
| Field | Type / values | Default | Meaning |
| --- | --- | --- | --- |
| `media_providers.providers` | dynamic object | `{}` | Alist, Emby, Bilibili, and other provider instance configuration |
| `media_providers.request_timeout_seconds` | seconds | `30` | Total provider HTTP request timeout |
| `media_providers.connect_timeout_seconds` | seconds | `10` | Provider HTTP connection timeout |
