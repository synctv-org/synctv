...
impl DouyinPlaybackProviderService for DouyinPlaybackProviderGrpcService {
    ...
    async fn get_resource(
        ...
        execute_playback_provider_stream(state.clone(), metadata, move |request_control| {
            ...
            async move {
                crate::impls::playback_provider::douyin::get_douyin_resource(
    ...
    async fn get_segment(
        ...
        execute_playback_provider_stream(state.clone(), metadata, move |request_control| {
            let state = state.clone();
            async move {
                crate::impls::playback_provider::douyin::get_douyin_segment(
                    deps(&state, Some(&request_control)),
                    req,
                )
                .await
    ...
    async fn watch_danmaku(
        ...
        execute_playback_provider_stream(state.clone(), metadata, move |request_control| {
            let state = state.clone();
            async move {
                crate::impls::playback_provider::douyin::watch_douyin_danmaku(
                    deps(&state, Some(&request_control)),
                    req,
                )
                .await
...
) -> crate::impls::playback_provider::douyin::DouyinPlaybackProviderDeps<'a> {
    crate::impls::playback_provider::douyin::DouyinPlaybackProviderDeps {
