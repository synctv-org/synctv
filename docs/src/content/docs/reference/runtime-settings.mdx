...
| `permissions.guestDefaultPermissions` | 权限名称数组 | `[]` | 房间 guest 全局默认权限；只允许 `view_members`、`view_chat_history`、`use_webrtc` |

这些设置使用 JSON 数组保存稳定的权限名称，例如：

```json
["send_chat_messages", "manage_own_media", "view_media", "view_members"]
```

`permissions.guestDefaultPermissions` 使用独立 guest 上限，不能写入 `view_media`、`send_chat_messages`、媒体资源、播放控制或管理权限。允许的示例：

```json
["view_members", "view_chat_history", "use_webrtc"]
```
