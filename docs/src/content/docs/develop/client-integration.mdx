...
| 展示同步播放 | 读取当前播放状态、获取播放信息、订阅 `playback_state` 和 `playback` | [播放模型](../../concepts/playback-model/) |
...
| 创建附件上传会话 | `POST /api/rooms/{room_id}/chat/attachments/upload-session` |
...
实时资源观察用于在房间 WebSocket 上订阅可缓存资源，例如播放状态、播放信息、房间设置、播放列表项和房间成员。客户端发送 `ClientMessage.observe_resource`，服务端返回 `resource_observed`、`resource_changed` 或 `resource_observe_error`。
