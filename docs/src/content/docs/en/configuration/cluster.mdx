...
<Aside type="caution">
Cluster mode cannot compensate for inconsistent dependencies. Replicas must share the same PostgreSQL, Redis, and `server.cluster_secret`. Local HLS backends work through publisher-node proxying, so publisher-node restarts, reachability issues, or network partitions affect remote segment reads. `shared_file` improves TS segment routing by reading from the current node's shared mount; OSS gives object-storage-backed segment storage.
</Aside>
