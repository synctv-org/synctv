...
SyncTV 默认允许访问私有网络媒体源、代理目标和远程视频源。自托管场景中，Alist、Emby、NAS、IPTV 和反向代理经常位于内网。

这意味着私有地址 SSRF 拦截不是全局默认安全边界。需要严格出站控制的调用点应显式使用 strict policy。

公网部署应通过反向代理、NetworkPolicy、防火墙和 Provider 凭据范围限制可达网络。不要把内部管理端点作为普通媒体源暴露给不可信用户。
