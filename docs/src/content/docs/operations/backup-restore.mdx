...
| 生产 secret | 必须 | 认证、数据解密、签名、集群通信和管理入口依赖这些值 |
...
<Aside type="caution">
`security.opaque_server_setup_secret`、Provider 凭据 key 和 TOTP key 与 PostgreSQL 中的持久数据绑定。恢复时必须使用备份时的值。
</Aside>
...
| `security.totp_encryption_key` | Secret Manager、离线加密备份 |
| `security.email_outbox_encryption_key` | Secret Manager、离线加密备份 |
| 代理、媒体 swarm、Provider 会话、登录发现、WebAuthn、文件 token key | Secret Manager |
