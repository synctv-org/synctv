...
//!
//! Tests for the `derive_api_address` helper function used to normalize
//! peer entries onto the shared API port.

#![allow(clippy::unwrap_used)]
// StaticDiscovery::derive_api_address is a private method.
// We test the same logic by reconstructing the function behavior directly.

/// Replicate the `derive_api_address` logic from `StaticDiscovery`.
fn derive_api_address(peer_address: &str, default_api_port: u16) -> String {
    peer_address.rfind(':').map_or_else(
...
#[test]
fn test_derive_api_address_replaces_port() {
    let result = derive_api_address("10.0.0.1:50051", 8080);
    assert_eq!(result, "10.0.0.1:8080");
}

#[test]
fn test_derive_api_address_no_port() {
    let result = derive_api_address("10.0.0.1", 8080);
    assert_eq!(result, "10.0.0.1:8080");
}

#[test]
fn test_derive_api_address_hostname() {
    let result = derive_api_address("my-host:50051", 9090);
    assert_eq!(result, "my-host:9090");
}

#[test]
fn test_derive_api_address_ipv6() {
    // For IPv6 like [::1]:50051, rfind(':') will find the port separator
    let result = derive_api_address("[::1]:50051", 8080);
...
fn test_static_discovery_config_defaults() {
    ...
    assert_eq!(config.default_api_port, 8080);
