...
启用后，Redis 和 `server.cluster_secret` 必填。
...
<Aside type="caution">
集群模式不能修复底层依赖不一致。多个副本必须共享同一个 PostgreSQL、Redis 和 `server.cluster_secret`。HLS 本地 backend 可以通过 publisher-node proxy 工作，因此 publisher 节点不可达、重启或网络分区时，远端节点读取分片会受影响；`shared_file` 会让 TS 分片从当前节点挂载的共享路径读取，OSS 则提供对象存储分片。
</Aside>
...
- Redis 可用。
- `server.cluster_secret` 稳定且所有副本一致。
