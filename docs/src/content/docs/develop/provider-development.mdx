...
3. **复用稳定核心契约。** `MediaProvider`、`DynamicFolder`、`PlaybackResult`、`SourceConfig`、`ProviderTarget`、凭据仓库和 transport action 是跨 Provider 共享边界。
...
4. 实现 Core `MediaProvider`，需要浏览时实现 `DynamicFolder`。
