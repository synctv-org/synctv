---
title: 房间、权限与用户偏好
description: SyncTV 的全局用户角色、房间角色、权限位、房间设置、成员权限覆盖和用户偏好语义。
---
...
```text
全局角色默认权限
  -> 房间设置 added/removed 权限
  -> 成员个人 added/removed 权限
```
...
房间设置控制加入策略、显示能力、默认权限和 RTMP 播放能力。
...
| `rtmp_player` | `false` | 是否允许 RTMP 播放，默认关闭 |
...
<Aside type="caution">
`rtmp_player` 默认关闭，因为 RTMP 播放没有同等认证语义。普通观看路径优先使用 HTTP-FLV、HLS 或客户端支持的安全播放路径。
</Aside>
...
<CardGrid>
  <Card title="少用 bitmask 直改" icon="setting">
