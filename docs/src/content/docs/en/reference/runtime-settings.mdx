...
Updates use protobuf `FieldMask`. Keys under `settings` and the JSON form of `updateMask` use lowerCamel names. Separate multiple mask paths with commas:
...
An optional field named by the mask is cleared when it is absent from `settings`. Required fields named by the mask must have a value. The mask accepts leaf paths only.
