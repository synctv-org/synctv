...
Guests receive only basic realtime room state by default. They cannot read playlists, send chat or danmaku, or manage media. A room may grant guest-safe capabilities through `guest_added_permissions`, such as member list, chat history, or WebRTC signaling; this ceiling is independent from member permissions. Even with extra room permissions, guests cannot use account, playlist, media management, member administration, room administration, Provider credential, or management-plane APIs.
