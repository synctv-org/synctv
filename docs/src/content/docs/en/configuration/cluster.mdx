...
<Diagram
  ...
  caption="In cluster mode, durable business state goes to PostgreSQL and cross-node runtime state goes to Redis. HLS can use local backends with publisher-node proxying, or shared filesystem/OSS backends for direct segment reads from every replica."
/>
...
<Aside type="caution">
Cluster mode cannot compensate for inconsistent dependencies. Replicas must share the same PostgreSQL, Redis, and `server.cluster_secret`. Local HLS backends work through publisher-node proxying, but publisher-node restarts, reachability issues, or network partitions affect remote segment reads; shared filesystem or OSS backends reduce that cross-node dependency.
</Aside>
