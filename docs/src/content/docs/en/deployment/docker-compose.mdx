...
Development Compose depends on the repository Dockerfile and source tree. Start from a full checkout:

<Code code={`git clone ${githubCloneUrl} synctv
cd synctv`} lang="bash" />

For production single-node deployment, you can download the minimum required files into an empty directory:

<Code code={`mkdir synctv-compose
cd synctv-compose
...
chmod +x scripts/init-compose-env.sh`} lang="bash" />
