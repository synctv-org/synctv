...
Implement sign-in, room list, ticket creation, Realtime, and playback snapshots first. Then add Providers, chat, danmaku, notifications, and WebRTC.
...
Business APIs use Bearer tokens. Signed-in users send access tokens; guests send guest tokens when reading public room resources:
...
- Use guest tokens only for the public room they were issued for. They can read or participate in guest-enabled room resources, but cannot log in, refresh a session, log out, or call account APIs.
...
<Aside type="note">
OPAQUE is the local password login protocol. When a password first factor triggers MFA, clients should complete the challenge with an available non-password method such as email or WebAuthn.
</Aside>
...
Local password login is OPAQUE-only. Generate the credential request in your client and call `StartOpaqueLogin`, then finish with `FinishOpaqueLogin`:
