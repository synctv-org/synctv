...
| `docker-compose.yml` | Image-based deployment | Uses `synctvorg/synctv:0.1.0` and requires explicit production secrets |
...
If a required variable is missing, Compose refuses to render the final config.
