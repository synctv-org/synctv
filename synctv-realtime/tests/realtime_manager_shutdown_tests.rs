//! Tests for `RealtimeManager` shutdown behavior
//!
...
/// Helper to create a `RealtimeManager` in single-node mode (no Redis)
async fn make_realtime_manager(node_id: &str) -> RealtimeManager {
...
async fn test_shutdown_waits_for_heartbeat_task_completion() {
    let manager = make_realtime_manager("shutdown-test-node").await;
...
async fn test_shutdown_has_timeout_for_heartbeat_task() {
    let manager = make_realtime_manager("timeout-test-node").await;
...
async fn test_shutdown_handles_task_error_gracefully() {
    let manager = make_realtime_manager("error-test-node").await;
...
async fn test_shutdown_without_heartbeat_task() {
    let manager = make_realtime_manager("no-heartbeat-node").await;
...
async fn test_shutdown_is_idempotent() {
    let manager = make_realtime_manager("idempotent-node").await;
...
async fn test_shutdown_completes_within_timeout() {
    let manager = make_realtime_manager("timeout-complete-node").await;
...
async fn test_shutdown_pattern_matches_publisher_task() {
    let manager = make_realtime_manager("pattern-test-node").await;
...
/// Test that cancelling a parent token also cancels the RealtimeManager's
...
async fn test_parent_cancel_token_propagates_cancellation() {
    ...
    assert!(
        manager.cancel_token().is_cancelled(),
        "Cancelling parent token should propagate to RealtimeManager's child token"
    );
}

// Test 9: without parent cancel token, RealtimeManager uses independent token
