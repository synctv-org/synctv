...
6. 如果 HLS backend 是 `file + hls_shared_storage=true` 或 `oss`，任意节点也可以直接从共享 backend 读取分片，减少对 publisher 节点的回源压力。
</Steps>

<Aside type="note">
多副本 HLS 有两种模型：本地 backend 依赖 publisher-node proxy，部署简单但远端 segment 请求会穿过 publisher 节点；共享文件系统或 OSS 让各节点直接读取分片，更适合生产高并发和滚动升级边界。
</Aside>
