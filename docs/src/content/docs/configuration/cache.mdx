...
| 字段 | 默认值 | 调整场景 | 风险 |
| --- | --- | --- | --- |
| `cache.l1_capacity` | `500` | 单节点内存充足且用户/房间对象 churn 明显时 | 太小会增加数据库查询；太大增加内存占用 |
...
| `cache.username_cache_capacity` | `1000` | 登录、成员列表或展示页频繁按用户名查询时 | 太小会造成用户名查询反复回源数据库 |
| `cache.username_cache_ttl_seconds` | `3600` | 用户名变更频率和查询压力不匹配时 | 太长会让用户名变更传播慢；太短会降低命中率 |
| `cache.permission_cache_capacity` | `1000` | 权限相关缓存容量需要统一调优时 | 太小会增加权限读取开销 |
