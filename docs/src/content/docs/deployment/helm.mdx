...
Helm 是 Kubernetes 多副本和平台化运维路径，不是第一次接触 SyncTV 的默认入口。只有当你已经准备好 Kubernetes 的 Secret、PVC、Ingress、ServiceMonitor/VMServiceScrape、滚动更新和数据库/Redis 运维边界时，才应该从这页开始。
...
| Redis | 生产推荐，多副本必需 | 多副本不能使用不同 Redis 或冲突 key prefix |
...
Helm 默认不启用集群模式。多副本 HLS 可以先使用 publisher-node HLS proxy；生产高流量场景建议使用 `shared_file` 或 OSS。`shared_file` 的 TS 分片由当前节点从共享路径读取。
