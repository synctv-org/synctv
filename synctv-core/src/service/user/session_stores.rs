...
use super::session_types::{
    LoginSession, MfaSession, OpaqueRegistrationSession, SensitiveVerificationSession,
};
...
use redis::{
    RedisLoginSessionStore, RedisMfaSessionStore, RedisOpaqueRegistrationSessionStore,
...
#[must_use]
pub fn local_login_session_store() -> Arc<dyn LoginSessionStore> {
...
fn shared_login_session_store(
    runtime: Arc<dyn RedisConnectionRuntime>,
    key_prefix: impl Into<String>,
) -> Arc<dyn LoginSessionStore> {
    Arc::new(RedisLoginSessionStore::from_runtime(runtime, key_prefix))
}
...
pub fn login_session_store_from_shared_state_profile(
    profile: &SharedStateProfile,
) -> Result<Arc<dyn LoginSessionStore>> {
    match profile.state_mode() {
        SharedStateMode::SharedRequired => {
            let runtime = profile.require_shared_runtime("login session storage")?;
            Ok(shared_login_session_store(
                runtime,
                profile.key_prefix().to_string(),
            ))
        }
        SharedStateMode::SharedBestEffort => Ok(shared_login_session_store(
            profile.best_effort_shared_runtime("login session storage")?,
            profile.key_prefix().to_string(),
        )),
        SharedStateMode::LocalOnly => Ok(local_login_session_store()),
    }
}
