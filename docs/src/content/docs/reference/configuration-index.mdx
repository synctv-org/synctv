...
字段默认值以源码默认配置为准。OAuth2 provider 实例属于 runtime settings，见 [运行时设置](../runtime-settings/#oauth2)。远程 media provider instance 通过管理 API/CLI 持久化，不属于静态配置文件。

## 完整配置示例

配置示例不直接塞进本页，避免索引过长。建议使用以下入口：

- [完整配置示例](../../configuration/full-example/)：最小生产配置骨架、覆盖全部静态字段的全量 YAML 模板、URL/拆分配置示例。
- `synctv config show --output yaml`：输出当前二进制支持的完整有效配置，并自动打码 secret。

## 先做配置决策

如果你不是在查某个字段，先用这张表判断“现在该不该改”。绝大多数字段保留默认值即可，生产上线优先处理 secret、数据库、入口、安全边界和备份。
...
如果你使用生产 Compose，优先维护 `.env.postgres` 和 `.env.synctv`，通常不需要手写完整 YAML。上线前重点确认：
...
当你离开 Compose 路径，改用裸机、Kubernetes、secret file 或自定义配置文件时，再使用下面的字段索引逐项查配置。
