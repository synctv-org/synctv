...
<Diagram
  ...
  caption="集群模式下，业务持久状态进入 PostgreSQL，跨节点运行时状态进入 Redis。本地 HLS backend 使用 publisher-node proxy；shared_file 会让当前节点从共享挂载读取 TS 分片，S3 则提供对象存储边界。"
/>
...
<Aside type="caution">
集群模式不能修复底层依赖不一致。多个副本必须共享同一个 PostgreSQL、Redis 和 `cluster.secret`。HLS 本地 backend 可以通过 publisher-node proxy 工作，因此 publisher 节点不可达、重启或网络分区时，远端节点读取分片会受影响；`shared_file` 会让 TS 分片从当前节点挂载的共享路径读取，S3 则提供对象存储分片。
</Aside>
...
| 多副本直播，高流量 | HLS 请求量高、需要更稳定的滚动升级边界 | 在集群配置基础上增加 `shared_file` 或 `s3` HLS backend |
...
如果 peer 不写端口，SyncTV 使用 `cluster.port`。
...
`s3` backend 使用 S3 兼容对象存储。

## Helm 中的集群建议

Helm chart 默认不启用集群模式。扩容前显式设置 `config.cluster.enabled=true`。HLS 可以先使用 publisher-node proxy；需要更稳定的分片存储和恢复边界时，使用 `shared_file` 或 `s3`。

扩容前必须确认：

- Redis 可用。
- `cluster.secret` 稳定且所有副本一致。
- HLS 模型已明确：小规模使用本地 backend + publisher-node proxy；需要持久共享存储时使用 `shared_file + RWX/PVC` 或 `s3`。
