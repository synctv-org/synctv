...
4. HLS 分片写入 `memory`、`file` 或 `oss` backend。
...
6. 当前跨节点 HLS 读取以 publisher-node HLS gRPC proxy 为主路径；共享 `file` 或 `oss` backend 表示分片存储边界更稳定，但不等于非 publisher 节点会绕过 publisher 直接读取。
</Steps>

<Aside type="note">
多副本 HLS 的当前实现依赖 publisher registry 定位推流节点，并通过 HLS gRPC proxy 读取远端 playlist/segment。共享文件系统或 OSS 可以让存储更持久、更适合滚动升级后的恢复边界，但容量规划仍要把 publisher-node proxy 的回源压力算进去。
</Aside>
...
多副本部署时，如果 HLS 文件写在本地磁盘，非 publisher 节点会通过 HLS gRPC proxy 向 publisher 节点读取。共享文件 backend 可以避免把分片锁死在单个 Pod 的本地磁盘上，并让重启后的节点仍能看到同一份 HLS 文件；但当前请求路径仍以 publisher-node proxy 为准。
