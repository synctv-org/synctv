...
pub mod pagination;
pub mod playback;
pub mod playlist_items_snapshot;
pub mod request_context;
pub mod room_members_snapshot;
...
pub fn normalize_member_remark_name(value: impl AsRef<str>) -> String {
    value.as_ref().trim().to_string()
}

pub fn normalize_member_display_tag(value: impl AsRef<str>) -> String {
    value.as_ref().trim().to_string()
}
...
pub fn disabled_provider_access_service() -> Arc<dyn synctv_core::provider::ProviderAccessService> {
...
pub fn proto_page_size_u32_usize(
    page_size: u32,
    default_page_size: u32,
    max_page_size: u32,
) -> Result<usize, ApiError> {
...
pub fn playlist_media_count_or_zero<S: std::hash::BuildHasher>(
    counts: &std::collections::HashMap<synctv_core::models::PlaylistId, i64, S>,
    playlist_id: &synctv_core::models::PlaylistId,
) -> i64 {
    counts.get(playlist_id).copied().unwrap_or(0)
}

pub fn room_member_count_or_zero<S: std::hash::BuildHasher>(
    counts: &std::collections::HashMap<synctv_core::models::RoomId, i32, S>,
    room_id: &synctv_core::models::RoomId,
) -> i32 {
...
pub use synctv_adapter::error::error_codes;
...
pub fn find_livestream_stream_error<'a>(
...
#[must_use]
pub fn map_livestream_backend_error(error: &(dyn std::error::Error + 'static)) -> ApiError {
