...
#[cfg(feature = "openapi")]
use synctv_proto::playback_provider::twitch::TwitchChatEvent;
use synctv_proto::playback_provider::twitch::{
    GetTwitchResourceRequest, GetTwitchSegmentRequest, TwitchResourceResponse,
...
impl PlaybackProviderHttpResponse for TwitchResourceResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}

impl PlaybackProviderHttpResponse for TwitchSegmentResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}
...
pub fn get_twitch_resource(
    Path(path): Path<TwitchResourcePath>,
...
async fn twitch_resource(
    path: TwitchResourcePath,
    state: AppState,
    request_meta: RequestMetadata,
    headers: HeaderMap,
    query_string: String,
    ...
    stream_http_response::<TwitchResourceResponse, _>(
        state,
        request_meta,
        method,
        move |request_control| {
            ...
            async move {
                crate::impls::playback_provider::twitch::get_twitch_resource(
...
async fn twitch_segment(
    ...
    let state_for_stream = state.clone();
    stream_http_response::<TwitchSegmentResponse, _>(
        state,
        request_meta,
        method,
        move |request_control| {
            let state = state_for_stream;
            async move {
                crate::impls::playback_provider::twitch::get_twitch_segment(
                    twitch_deps(&state, Some(&request_control)),
                    req,
                )
                .await
...
#[cfg_attr(
    ...
    utoipa::path(
        ...
        responses((
            status = 200,
            description = "Twitch IRC chat SSE",
            body = TwitchChatEvent,
...
pub async fn watch_twitch_chat(
    ...
    let stream = state
        ...
        .execute_public_with_control(
            ...
            move |request_control| {
                let state = state_for_stream;
                async move {
                    crate::impls::playback_provider::twitch::watch_twitch_chat(
                        twitch_deps(&state, Some(&request_control)),
                        req,
                    )
                    .await
    ...
    Ok(
        Sse::new(stream.map(super::transport::twitch_chat_sse_event).boxed())
...
) -> crate::impls::playback_provider::twitch::TwitchPlaybackProviderDeps<'a> {
    crate::impls::playback_provider::twitch::TwitchPlaybackProviderDeps {
