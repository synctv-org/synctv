...
synctv settings update --request-json '{"settings":{"user":{"enablePasswordSignup":true}},"updateMask":"user.enablePasswordSignup"}'
```

更新协议使用 protobuf `FieldMask`。`settings` 中的 key 和 `updateMask` 的 JSON 表示均为 lowerCamel；多个路径用逗号分隔：

```bash
synctv settings update --request-json '{"settings":{"email":{"whitelistEnabled":true,"whitelistDomains":["example.com"]}},"updateMask":"email.whitelistEnabled,email.whitelistDomains"}'
```

mask 中存在的可选字段在 `settings` 中缺失时会被清除。普通字段必须在 `settings` 中提供值。mask 只接受叶子路径。
...
`permissions.guestDefaultPermissions` 使用独立 guest 上限，不能写入 `view_media_resources`、`send_chat`、媒体资源、播放控制或管理权限。允许的示例：
...
```bash
synctv settings update --request-json '{"settings":{"user":{"passwordSignupNeedReview":true}},"updateMask":"user.passwordSignupNeedReview"}'
```
...
```bash
synctv settings update --request-json '{"settings":{"roomCreation":{"maxRoomsPerUser":"20"}},"updateMask":"roomCreation.maxRoomsPerUser"}'
```
...
```bash
synctv settings update --request-json '{"settings":{"chat":{"maxMessagesPerRoom":"1000","messageRetentionDays":"180"}},"updateMask":"chat.maxMessagesPerRoom,chat.messageRetentionDays"}'
```
...
```bash
synctv settings update --request-json '{"settings":{"oauth2":{"providers":[{"name":"github","enableSignup":true,"signupNeedReview":false,"github":{"clientId":"github-client-id","clientSecret":"github-client-secret","redirectUrl":"https://app.example.com/oauth2/callback"}}]}},"updateMask":"oauth2.providers"}'
```
