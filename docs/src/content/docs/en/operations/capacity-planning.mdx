...
3. Estimate login, refresh, Provider browse, playback snapshot, and list requests per minute.
...
| `http_rate_limits.websocket_*` | WebSocket connection-attempt limits |
| `http_rate_limits.*` | HTTP login, API, media, admin, and streaming limits |
| `grpc_rate_limits.*` | gRPC API and verification limits |
| `messaging_rate_limits.*` | Chat and danmaku message limits |
...
<Tabs syncKey="capacity-tests-en">
  <TabItem label="API" icon="document">
    Test real login, room lists, member lists, playback snapshots, and Provider browse paths. Do not only load test `/health/ready`.
  </TabItem>
