...
- 启动或配置错误：先看 [排障入口](../troubleshooting/)。
- 容量、连接数、数据库、Redis 或媒体带宽规划：看 [容量规划](../capacity-planning/)。
- 指标名称和分组：看 [Metrics Catalog](../../reference/metrics-catalog/)。
- 上线前检查：看 [生产部署清单](../../install/production-checklist/)。
- 数据恢复：看 [备份与恢复](../backup-restore/)。
- 版本发布：看 [升级与迁移](../upgrades/)。
- metrics 配置：看 [Metrics 监控](../../configuration/metrics/)。
