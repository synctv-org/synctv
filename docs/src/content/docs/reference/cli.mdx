...
synctv media add --room-id <ROOM_ID> --username alice --source-provider direct_url --source-config-json '{"medias":[{"url":"https://example.com/video.mp4"}]}' --name "视频"
...
synctv settings update permissions --set guest_default='["view_member_list","use_webrtc"]'
