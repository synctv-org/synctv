...
| `required variable ... is missing` | Production Compose is missing `.env.postgres` or `.env.synctv` | Run `./scripts/init-compose-env.sh` |
...
- HTTP error `request_id`, `status`, and `code`.
