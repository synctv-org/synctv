...
impl PlaybackProviderHttpResponse for CloudreveResourceResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}

impl PlaybackProviderHttpResponse for CloudreveHlsManifestResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}

impl PlaybackProviderHttpResponse for CloudreveHlsResourceResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}

impl PlaybackProviderHttpResponse for CloudreveSubtitleResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}
...
pub fn head_cloudreve_resource(
    Path(path): Path<CloudreveResourcePath>,
    State(state): State<AppState>,
    request_meta: RequestMetadata,
    headers: HeaderMap,
...
async fn resource(
    ...
    stream_http_response::<CloudreveResourceResponse, _>(
        state,
        request_meta,
        method,
        move |control| {
...
pub async fn head_cloudreve_hls_manifest(
    Path(path): Path<CloudreveResourcePath>,
    State(state): State<AppState>,
    request_meta: RequestMetadata,
    raw_query: RawQuery,
...
async fn hls_manifest(
    ...
    let stream_state = state.clone();
    stream_http_response::<CloudreveHlsManifestResponse, _>(
        state,
        request_meta,
        method,
        move |control| {
...
pub fn head_cloudreve_hls_resource(
    Path(path): Path<CloudreveHlsResourcePath>,
    State(state): State<AppState>,
    request_meta: RequestMetadata,
    headers: HeaderMap,
...
async fn hls_resource(
    ...
    let req = GetCloudreveHlsResourceRequest {
        ...
        mode_name: path.mode_name,
        media_index: path.media_index,
        resource_kind: cloudreve_hls_resource_kind(&path.resource_kind)?,
    };
    let stream_state = state.clone();
    stream_http_response::<CloudreveHlsResourceResponse, _>(
        state,
        request_meta,
        method,
        move |control| {
...
fn cloudreve_hls_resource_kind(value: &str) -> AppResult<i32> {
    match value {
        "media" => Ok(CloudreveHlsResourceKind::Media as i32),
        "manifest" => Ok(CloudreveHlsResourceKind::Manifest as i32),
        _ => Err(crate::http::error::map_api_error(
            synctv_api_common::impls::ApiError::InvalidInput(
                "Invalid Cloudreve HLS resource kind".to_string(),
            ),
        )),
    }
}
...
pub async fn get_cloudreve_subtitle(
    ...
    let stream_state = state.clone();
    stream_http_response::<CloudreveSubtitleResponse, _>(
        state,
        request_meta,
        Method::GET,
        move |control| {
