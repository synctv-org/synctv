---
title: 播放与代理模型
description: SyncTV 播放快照、播放 URL、Provider header、代理模式、Range、slice cache、WebRTC 和直播播放边界。
---
...
<Steps>
1. 客户端请求房间当前播放或指定媒体的播放快照。
2. SyncTV 根据房间状态、媒体、用户、Provider 凭据和客户端能力生成 `PlaybackSnapshot`。
...
新客户端使用 Realtime 资源观察订阅播放快照：

- `playback_state`：当前播放位置、状态和版本。
- `playback_snapshot`：当前媒体的播放 URL、headers、字幕和过期时间。
...
| 播放 URL 一段时间后失效 | `expires_at` | 订阅或重新拉取播放快照 |
