...
<Aside type="caution">
OPAQUE, Provider credentials, and the email outbox use separate long-lived secrets. Back up every secret together with PostgreSQL.
</Aside>
...
| Check | Acceptance criteria |
| --- | --- |
| HTTP entrypoint | The [Nginx reverse proxy](../nginx/) or Ingress supports WebSocket upgrades; Nginx proxy caching and response buffering are disabled. |
