...
/// Default aggregate timeout for fan-out operations across all nodes.
...
pub struct ClusterClientConfig {
    ...
    /// Total timeout for collecting one fan-out operation across all nodes
...
impl ClusterClient {
    ...
    /// can use the returned `api_address` to invoke their own business gRPC
    /// services directly.
    pub async fn resolve_routable_node(
        &self,
        target_node_id: &str,
    ) -> Result<crate::discovery::NodeInfo> {
        self.find_routable_node(target_node_id).await
    }

    /// Return all routable remote nodes, excluding this client node.
    pub async fn remote_routable_nodes(&self) -> Result<Vec<crate::discovery::NodeInfo>> {
        let (nodes, _view_mode) = self.node_registry.get_routable_nodes().await?;
        Ok(nodes
            .into_iter()
        ...
        ExtractFn: Fn(T) -> Vec<Item>,
    {
        ...
        let aggregate_timeout = self.config.aggregate_timeout;
        ...
        if timed_out {
            ...
            failures.extend(pending_nodes.into_iter().map(|(node_id, address)| {
                (
                    node_id,
                    format!(
                        "aggregate timeout after {aggregate_timeout:?} while waiting for {address}"
                    ),
                )
            }));
            warn!(
...
mod tests {
    ...
    #[test]
    fn test_cluster_client_config_default_aggregate_timeout_matches_legacy_default() {
        let config = ClusterClientConfig::default();

        assert_eq!(config.aggregate_timeout, DEFAULT_FAN_OUT_AGGREGATE_TIMEOUT);
    }
}
