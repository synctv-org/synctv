...
路径中的房间 ID 属于签名 claim。签名播放查询包含 `sig`、`uid` 和 `exp`。重写后的 HLS 资源还包含已签名的 `targetUrl`。DASH 资源路由在路径中携带签名字段，上游资源查询保持独立。这个布局保留上游查询，并让公开路径只保留一个房间标识。
...
客户端通过 `PlaybackClientProfile.supported_live_transports` 声明播放器支持的直播传输。当前协议定义 `PLAYBACK_LIVE_TRANSPORT_HLS` 和 `PLAYBACK_LIVE_TRANSPORT_FLV`。Provider 在请求上游播放信息前选择传输，并让上游请求参数、`PlaybackMedia.format` 和实际 URL 保持一致。
