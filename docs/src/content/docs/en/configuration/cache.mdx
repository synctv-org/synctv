...
| --- | --- | --- | --- |
| `cache.l1_capacity` | `500` | One node has enough memory and user/room cache churn is visible | Too low increases database reads; too high increases memory use |
...
| `cache.username_cache_capacity` | `1000` | Login, member lists, or display paths frequently resolve usernames | Too low repeatedly falls back to the database |
...
| `cache.permission_cache_capacity` | `1000` | Permission cache capacity needs explicit tuning | Too low increases permission-read overhead |
...
## Single-Node Production Guidance
