...
[Quick Start](../../install/quick-start/) production Compose uses `.env.postgres` and `.env.synctv`. Confirm at least:
...
<Aside type="note">
Production Compose users should maintain `.env.postgres` and `.env.synctv`; do not maintain a second copy of YAML secrets.
</Aside>
