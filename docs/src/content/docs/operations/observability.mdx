...
- **日志管线**：`logging_dropped_lines_total{component=...}` 应保持为零；数值增长表示对应组件的 non-blocking 队列达到容量上限。
...
| `logging_dropped_lines_total` 增长 | 日志输出端变慢，或组件产生日志的速度持续高于输出速度 | 组件日志级别、磁盘或采集器延迟、组件输出配置 |
