...
- 文件和图片走 `CreateChatAttachmentUploadSession`，再把返回的 `ChatAttachment` 放进 `SendChatMessage.attachments`。请求必须带 `checksum_sha256`，服务端用全局内容寻址对象实现复用，并按 MIME 类型推断 `kind`；`upload_required=true` 时先按 `upload_url`、`upload_method`、`upload_headers` 上传对象；`ownership_proof_required=true` 时按返回的 `ownership_proof_nonce` 和 `ownership_proof_ranges` 从本地文件读取字节，计算 SHA-256 hex，并写入 `attachment.metadata[ownership_proof_metadata_key]` 后发送。proof 哈希输入顺序为 `synctv-file-ownership-proof-v1`、一个 `0x00` 字节、nonce UTF-8、每个 range 的 `offset` big-endian i64、`length` big-endian i32、range 字节。
