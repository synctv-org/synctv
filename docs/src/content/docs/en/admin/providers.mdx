...
Remote Provider instances can currently declare `alist`, `bilibili`, and `emby`. Media sources also include local types such as `direct_url`, `rtmp`, and `live_proxy`; those are not valid `--provider` values for remote Provider instances.
