...
use synctv_core::{
    models::{
        Room, RoomId, RoomMember, RoomRole, RoomStatus,
...
async fn test_admin_cannot_delete_room() {
    ...
    let user_repo = UserRepository::new(pool.clone());
    let member_repo = RoomMemberRepository::new(pool.clone());
    let admin_user = user_repo.create(&make_user("admin_delete")).await.expect("Failed to create admin");
    ...
    assert!(!admin_perms.has(PermissionBits::DELETE_ROOM), "Admin should not have DELETE_ROOM permission");
}
...
async fn test_admin_cannot_transfer_ownership() {
    ...
    let admin_user = user_repo.create(&make_user("admin_transfer")).await.expect("Failed to create admin");
    let admin_member = RoomMember::new(room.id.clone(), admin_user.id.clone(), RoomRole::Admin);
    member_repo.add(&admin_member).await.expect("Failed to add admin");

    let target_user = user_repo.create(&make_user("transfer_target")).await.expect("Failed to create target");
    let target_member = RoomMember::new(room.id.clone(), target_user.id.clone(), RoomRole::Member);
    member_repo.add(&target_member).await.expect("Failed to add target");

    // Admin tries to set Creator role (should fail)
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .set_member_role(room.id.clone(), admin_user.id.clone(), target_user.id.clone(), RoomRole::Creator)
        .await;
...
async fn test_admin_cannot_demote_creator() {
    ...
    let user_repo = UserRepository::new(pool.clone());
    let member_repo = RoomMemberRepository::new(pool.clone());
    let admin_user = user_repo.create(&make_user("admin_demote")).await.expect("Failed to create admin");
    let admin_member = RoomMember::new(room.id.clone(), admin_user.id.clone(), RoomRole::Admin);
    member_repo.add(&admin_member).await.expect("Failed to add admin");
    ...
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .set_member_role(room.id.clone(), admin_user.id.clone(), owner.id.clone(), RoomRole::Admin)
        .await;
...
async fn test_member_cannot_kick() {
    ...
    let kicker = user_repo.create(&make_user("member_kicker")).await.expect("Failed to create kicker");
    let kicker_member = RoomMember::new(room.id.clone(), kicker.id.clone(), RoomRole::Member);
    member_repo.add(&kicker_member).await.expect("Failed to add kicker");

    let target = user_repo.create(&make_user("kick_target")).await.expect("Failed to create target");
...
async fn test_member_cannot_ban() {
    ...
    let banner = user_repo.create(&make_user("member_banner")).await.expect("Failed to create banner");
    let banner_member = RoomMember::new(room.id.clone(), banner.id.clone(), RoomRole::Member);
    member_repo.add(&banner_member).await.expect("Failed to add banner");

    let target = user_repo.create(&make_user("ban_target")).await.expect("Failed to create target");
    let target_member = RoomMember::new(room.id.clone(), target.id.clone(), RoomRole::Member);
    member_repo.add(&target_member).await.expect("Failed to add target");
    ...
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .ban_member(room.id.clone(), banner.id.clone(), target.id.clone(), Some("Test ban".to_string()))
        .await;
...
async fn test_member_cannot_change_settings() {
    ...
    let member_user = user_repo.create(&make_user("settings_member")).await.expect("Failed to create member");
    ...
    assert!(!member_perms.has(PermissionBits::SET_ROOM_SETTINGS), "Member should not have SET_ROOM_SETTINGS permission");
}
...
async fn test_member_cannot_promote() {
    ...
    let promoter = user_repo.create(&make_user("member_promoter")).await.expect("Failed to create promoter");
    let promoter_member = RoomMember::new(room.id.clone(), promoter.id.clone(), RoomRole::Member);
    member_repo.add(&promoter_member).await.expect("Failed to add promoter");

    let target = user_repo.create(&make_user("promote_target")).await.expect("Failed to create target");
    let target_member = RoomMember::new(room.id.clone(), target.id.clone(), RoomRole::Member);
    member_repo.add(&target_member).await.expect("Failed to add target");
    ...
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .set_member_role(room.id.clone(), promoter.id.clone(), target.id.clone(), RoomRole::Admin)
        .await;
...
async fn test_cross_room_permission_isolation() {
    ...
    let cross_user = user_repo.create(&make_user("cross_room_user")).await.expect("Failed to create user");
    let admin_member_a = RoomMember::new(room_a.id.clone(), cross_user.id.clone(), RoomRole::Admin);
    member_repo.add(&admin_member_a).await.expect("Failed to add to Room A");
    ...
    member_repo.add(&member_b).await.expect("Failed to add to Room B");

    // Create a target member in Room B
    let target = user_repo.create(&make_user("room_b_target")).await.expect("Failed to create target");
...
async fn test_ban_isolated_to_single_room() {
    ...
    let banned_user = user_repo.create(&make_user("banned_user")).await.expect("Failed to create user");
    let member_a = RoomMember::new(room_a.id.clone(), banned_user.id.clone(), RoomRole::Member);
    member_repo.add(&member_a).await.expect("Failed to add to Room A");

    // Ban user from Room A
    let member_service = make_member_service(pool.clone());
    member_service
        .ban_member(room_a.id.clone(), owner_a.id.clone(), banned_user.id.clone(), Some("Test ban".to_string()))
        .await
    ...
    let result = member_service
        .add_member_with_options(room_b.id.clone(), banned_user.id.clone(), RoomRole::Member, AddMemberOptions::new())
        .await;
...
async fn test_creator_role_not_global() {
    ...
    let creator_user = user_repo.create(&make_user("creator_user")).await.expect("Failed to create creator");
    let room_a = {
        let room_repo = RoomRepository::new(pool.clone());
        let room = room_repo.create(&make_room("Creator Room", "Room where user is creator", &creator_user.id))
            .await
            .expect("Failed to create room");
        let member = RoomMember::new(room.id.clone(), creator_user.id.clone(), RoomRole::Creator);
        member_repo.add(&member).await.expect("Failed to add creator");
        room
    };
    ...
    assert!(perms_a.has(PermissionBits::DELETE_ROOM), "Creator should have DELETE_ROOM in Room A");
    assert!(!perms_b.has(PermissionBits::DELETE_ROOM), "Member should not have DELETE_ROOM in Room B");
}
...
async fn test_member_cannot_grant_self_permissions() {
    ...
    let member_user = user_repo.create(&make_user("self_granter")).await.expect("Failed to create member");
    let member = RoomMember::new(room.id.clone(), member_user.id.clone(), RoomRole::Member);
    member_repo.add(&member).await.expect("Failed to add member");
    ...
    assert!(result.is_err(), "Member cannot grant themselves permissions");
}
...
async fn test_admin_cannot_create_admin() {
    ...
    let admin_user = user_repo.create(&make_user("admin_creator")).await.expect("Failed to create admin");
    let admin_member = RoomMember::new(room.id.clone(), admin_user.id.clone(), RoomRole::Admin);
    member_repo.add(&admin_member).await.expect("Failed to add admin");

    let target_user = user_repo.create(&make_user("admin_target")).await.expect("Failed to create target");
    let target_member = RoomMember::new(room.id.clone(), target_user.id.clone(), RoomRole::Member);
    member_repo.add(&target_member).await.expect("Failed to add target");
    ...
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .set_member_role(room.id.clone(), admin_user.id.clone(), target_user.id.clone(), RoomRole::Admin)
        .await;
...
async fn test_grant_permission_requires_permission() {
    ...
    let grantor = user_repo.create(&make_user("grantor")).await.expect("Failed to create grantor");
    let grantor_member = RoomMember::new(room.id.clone(), grantor.id.clone(), RoomRole::Member);
    member_repo.add(&grantor_member).await.expect("Failed to add grantor");

    let target = user_repo.create(&make_user("grantee")).await.expect("Failed to create target");
    let target_member = RoomMember::new(room.id.clone(), target.id.clone(), RoomRole::Member);
    member_repo.add(&target_member).await.expect("Failed to add target");
    ...
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .grant_permission(room.id.clone(), grantor.id.clone(), target.id.clone(), PermissionBits::SEND_CHAT)
        .await;

    assert!(result.is_err(), "Member without GRANT_PERMISSION cannot grant");
}
...
async fn test_cannot_downgrade_equal_role() {
    ...
    let admin1 = user_repo.create(&make_user("admin1_downgrade")).await.expect("Failed to create admin1");
    let admin1_member = RoomMember::new(room.id.clone(), admin1.id.clone(), RoomRole::Admin);
    member_repo.add(&admin1_member).await.expect("Failed to add admin1");

    let admin2 = user_repo.create(&make_user("admin2_downgrade")).await.expect("Failed to create admin2");
    let admin2_member = RoomMember::new(room.id.clone(), admin2.id.clone(), RoomRole::Admin);
    member_repo.add(&admin2_member).await.expect("Failed to add admin2");

    // Admin1 tries to downgrade Admin2 (should fail - equal role)
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .set_member_role(room.id.clone(), admin1.id.clone(), admin2.id.clone(), RoomRole::Member)
        .await;
...
async fn test_kick_respects_role_hierarchy() {
    ...
    let admin = user_repo.create(&make_user("kick_admin")).await.expect("Failed to create admin");
    let admin_member = RoomMember::new(room.id.clone(), admin.id.clone(), RoomRole::Admin);
    member_repo.add(&admin_member).await.expect("Failed to add admin");

    let member = user_repo.create(&make_user("kick_member")).await.expect("Failed to create member");
    let member_member = RoomMember::new(room.id.clone(), member.id.clone(), RoomRole::Member);
    member_repo.add(&member_member).await.expect("Failed to add member");
...
async fn test_ban_respects_role_hierarchy() {
    ...
    let admin = user_repo.create(&make_user("ban_admin")).await.expect("Failed to create admin");
    let admin_member = RoomMember::new(room.id.clone(), admin.id.clone(), RoomRole::Admin);
    member_repo.add(&admin_member).await.expect("Failed to add admin");
    ...
    let member_service = make_member_service(pool.clone());
    let result = member_service
        .ban_member(room.id.clone(), admin.id.clone(), owner.id.clone(), Some("Test ban".to_string()))
        .await;
...
async fn test_revoked_permission_denied() {
    ...
    let member_user = user_repo.create(&make_user("revoke_member")).await.expect("Failed to create member");
    let member = RoomMember::new(room.id.clone(), member_user.id.clone(), RoomRole::Member);
    member_repo.add(&member).await.expect("Failed to add member");

    // Get owner for operations
    let members = member_repo.list_by_room_all(&room.id).await.expect("Failed to list members");
    let owner_user_id = members.iter()
    ...
    let member_service = make_member_service(pool.clone());
    member_service
        .revoke_permission(room.id.clone(), owner_user_id.clone(), member_user.id.clone(), PermissionBits::SEND_CHAT)
        .await
...
async fn test_member_without_send_chat_cannot_send() {
    ...
    let muted_user = user_repo.create(&make_user("muted_user")).await.expect("Failed to create muted user");
    let mut muted_member = RoomMember::new(room.id.clone(), muted_user.id.clone(), RoomRole::Member);
    // Revoke SEND_CHAT
    muted_member.removed_permissions = PermissionBits::SEND_CHAT;
    member_repo.add(&muted_member).await.expect("Failed to add muted user");
    ...
    assert!(!perms.has(PermissionBits::SEND_CHAT), "Muted user should not have SEND_CHAT");
}
