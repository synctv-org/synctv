...
| `file_storage.backends.<name>.database.compression` | `none`, `lz4`, or `zstd` | `zstd` | Compression algorithm used by `type: "database"` backends for persisted `file_blob_parts` segments |
| `file_storage.backends.<name>.database.compression_min_size_bytes` | bytes | `4096` | Minimum original payload size before compression is attempted |
| `file_storage.backends.<name>.database.compression_min_savings_percent` | `0` to `100` | `10` | Minimum saved percentage required before compressed bytes are stored |
| `file_storage.backends.<name>.s3.*` | S3 config | default S3 fields | S3-compatible settings for backends with `type: "s3"`; `public_base_url` is required for readable file URLs after upload or ownership proof validation |
