...
impl PlaybackProviderHttpResponse for TrueNasHlsManifestResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}

impl PlaybackProviderHttpResponse for TrueNasHlsResourceResponse {
    fn chunk(self) -> Option<synctv_proto::playback_provider::common::StreamChunk> {
        self.chunk
    }
}
...
async fn hls_manifest(
    ...
    let stream_state = state.clone();
    stream_http_response::<TrueNasHlsManifestResponse, _>(
        state,
        request_meta,
        method,
        move |control| {
...
async fn hls_resource(
    ...
    let req = GetTrueNasHlsResourceRequest {
        ...
        mode_name: path.mode_name,
        media_index: path.media_index,
        resource_kind: truenas_hls_resource_kind(&path.resource_kind)?,
    };
    let stream_state = state.clone();
    stream_http_response::<TrueNasHlsResourceResponse, _>(
        state,
        request_meta,
        method,
        move |control| {
...
fn truenas_hls_resource_kind(value: &str) -> AppResult<i32> {
    match value {
        "media" => Ok(TrueNasHlsResourceKind::Media as i32),
        "manifest" => Ok(TrueNasHlsResourceKind::Manifest as i32),
        _ => Err(crate::http::error::map_api_error(
            synctv_api_common::impls::ApiError::InvalidInput(
                "Invalid TrueNAS HLS resource kind".to_string(),
            ),
        )),
    }
}
...
mod tests {
    ...
    fn hls_resource_path_kind_maps_only_typed_routes() {
        assert_eq!(
            truenas_hls_resource_kind("media").expect("media route should be valid"),
            TrueNasHlsResourceKind::Media as i32
        );
        assert_eq!(
            truenas_hls_resource_kind("manifest").expect("manifest route should be valid"),
            TrueNasHlsResourceKind::Manifest as i32
        );
        assert!(truenas_hls_resource_kind("segment").is_err());
    }
}
