...
运行初始化脚本前，确认主机已安装 Docker Compose、`openssl` 和 `python3`。脚本使用 `openssl` 生成 PostgreSQL、JWT、cluster、credential-encryption 和 OPAQUE secret，并用 `python3` 安全地 URL-encode 数据库密码。
