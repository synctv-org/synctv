...
COMMENT ON COLUMN media.source_provider IS 'Media provider type name';
COMMENT ON COLUMN media.source_config IS 'Media provider-specific configuration (persistent)';
COMMENT ON COLUMN media.provider_instance_name IS 'Optional media provider instance name';
...
COMMENT ON CONSTRAINT media_name_length ON media IS 'Media display name must fit within the database column limit';
