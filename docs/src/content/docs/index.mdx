...
SyncTV 的日常入口是房间。用户在房间里一起看视频、聊天、发弹幕、切换播放列表；房间管理员控制成员和权限；平台管理员维护账号、审核、Provider、直播和运行策略。
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="使用 OpenAPI、gRPC、WebSocket Realtime、ticket、播放快照和错误码。"
  />
</CardGrid>
...
| 处理播放不同步或失败 | [同步观看](use/watch-together/) 和 [用户排障](use/troubleshooting/) |
...
<CardGrid>
  <Card title="房间" icon="puzzle">
    成员、播放列表、聊天、弹幕、权限和当前播放状态都围绕房间组织。见 [房间概念](concepts/rooms/)。
  </Card>
