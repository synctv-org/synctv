...
- `file_upload_sessions` stores resumable upload state for database multipart and S3 multipart.
...
S3 uploads default to direct client-to-S3 presigned URLs. Server-proxy upload/download paths remain available for environments that need them. Database backend URLs point at SyncTV because the database bytes live behind the application.
