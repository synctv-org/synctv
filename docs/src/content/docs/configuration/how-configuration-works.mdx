...
- `logging.output.path`
- `<component>.logging.output.path`，其中 `<component>` 为 `server`、`health`、`metrics`、`management`、`cluster`、`livestream` 或 `webrtc`
...
Runtime settings 通过管理 API、CLI 或导入快照加载时会忽略未知字段，同时继续执行已知字段的类型、范围和业务校验。这样可以让新版本导出的 settings 在旧版本上安全读取；静态启动配置仍按上面的严格规则处理。
