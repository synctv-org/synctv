...
fn map_oauth2_exchange_error(error: synctv_api_common::impls::ApiError) -> Status {
    map_api_error(error)
}
...
pub struct OAuth2GrpcService {
    oauth2_api: Arc<synctv_api_common::impls::OAuth2ApiImpl>,
...
mod tests {
    ...
    fn test_unlink_missing_binding_maps_to_grpc_not_found() {
        let status = map_api_error(synctv_api_common::impls::ApiError::NotFound(
