...
fn test_is_metadata_with_set_data_frame_and_on_metadata() {
    ...
    assert!(metadata.is_metadata(bytes), "Should accept @setDataFrame + onMetaData");
}
...
fn test_is_metadata_with_only_on_metadata() {
    ...
    let bytes = create_metadata_bytes(values);
    let mut metadata = MetaData::new();
    assert!(metadata.is_metadata(bytes), "Should accept onMetaData alone");
}
...
fn test_is_metadata_rejects_only_set_data_frame() {
    ...
    let bytes = create_metadata_bytes(values);
    let mut metadata = MetaData::new();
    assert!(!metadata.is_metadata(bytes), "Should reject @setDataFrame alone");
}
...
fn test_is_metadata_rejects_set_data_frame_with_wrong_second_value() {
    ...
    let bytes = create_metadata_bytes(values);
    let mut metadata = MetaData::new();
    assert!(!metadata.is_metadata(bytes), "Should reject @setDataFrame + wrong second value");
}
...
fn test_is_metadata_rejects_random_string() {
    ...
    let values = vec![
        Amf0ValueType::UTF8String("randomString".to_string()),
    ];
    let bytes = create_metadata_bytes(values);
    let mut metadata = MetaData::new();
...
fn test_is_metadata_with_additional_data() {
    ...
    let bytes = create_metadata_bytes(values);
    let mut metadata = MetaData::new();
    assert!(metadata.is_metadata(bytes), "Should accept onMetaData with properties object");
}
...
fn test_is_metadata_with_set_data_frame_and_properties() {
    ...
    let bytes = create_metadata_bytes(values);
    let mut metadata = MetaData::new();
    assert!(metadata.is_metadata(bytes), "Should accept @setDataFrame + onMetaData + properties");
}
