...
Production monitoring covers request handling, external dependencies, authentication, realtime connections, and media paths:
...
- **Logging**: `logging_dropped_lines_total{component=...}` should remain at zero; an increase means that component's non-blocking queue reached capacity.
