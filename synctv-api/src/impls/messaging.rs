...
use synctv_core::{
    models::{
        MediaId, PermissionBits, PlaylistId, RoomId, RoomPlaybackState, RoomStatus, UserId,
        UserStatus,
    },
    service::{
...
impl PlaybackSnapshotSource {
    ...
    fn matches_watch(
        ...
        let watch_media_id = if watch.media_id.trim().is_empty() {
            None
        } else {
            match public_id_codec.decode_media_id(&watch.media_id) {
                Ok(id) => Some(id),
                Err(_) => return false,
            }
        };
        let watch_playlist_id = if watch.playlist_id.trim().is_empty() {
            None
        } else {
            match public_id_codec.decode_playlist_id(&watch.playlist_id) {
                Ok(id) => Some(id),
                Err(_) => return false,
            }
        };

        self.media_id == watch_media_id
            && self.playlist_id == watch_playlist_id
...
impl StreamMessageHandler {
    ...
    async fn handle_watch_playback_state(&self, version: Option<i64>) -> Result<(), String> {
        let state = self
            .room_service
        ...
        if should_send {
            let room_id = self.public_room_id();
            self.send_server_message(ServerMessage {
                message: Some(
                    crate::proto::client::server_message::Message::PlaybackState(
                        crate::proto::client::PlaybackStateChanged {
                            room_id,
                            state: Some(playback_state_to_proto(&state, &self.public_id_codec)),
                        },
                    ),
                ),
            })?;
    ...
    async fn handle_watch_playback_snapshot(
        ...
        let should_send = watch.version.is_empty()
            ...
            || !current_source.matches_watch(watch, &self.public_id_codec);
    ...
    async fn handle_watch_playlist_items(
        ...
        if should_send {
            ...
            self.send_server_message(ServerMessage {
                message: Some(
                    crate::proto::client::server_message::Message::PlaylistItems(
    ...
    async fn current_playback_snapshot_version(
        ...
        if let Some(media_id) = &state.playing_media_id {
            let media = self
                ...
                .await
                .map_err(|error| error.to_string())?
                .ok_or_else(|| format!("Playing media '{media_id}' not found"))?;
        ...
        if let Some(playlist_id) = &state.playing_playlist_id {
            let playlist = self
                ...
                .await
                .map_err(|error| error.to_string())?
                .ok_or_else(|| format!("Playing playlist '{playlist_id}' not found"))?;
...
mod tests {
    ...
    struct MutableRoomSettingsSnapshotService {
        snapshot:
            Arc<parking_lot::Mutex<crate::impls::room_settings_snapshot::RoomSettingsSnapshot>>,
        probe: SnapshotCallProbe,
    }
    ...
    async fn test_watch_playback_state_without_version_sends_current_state_immediately() {
        ...
        let (mut stream, stream_state) = RecordingStream::with_incoming(vec![ClientMessage {
            message: Some(
                crate::proto::client::client_message::Message::WatchPlaybackState(
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                if message_sender
                    .sent_messages()
                    .iter()
                    .any(|message| matches!(message.message, Some(Message::PlaybackState(_))))
                {
                    break;
                }
                tokio::time::sleep(Duration::from_millis(10)).await;
    ...
    async fn test_watch_playback_snapshot_without_version_sends_snapshot_immediately() {
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_playback_snapshot_initial", message_sender.clone())
                .await;
        let StartTestFixture {
            handler,
            connection_service,
        ...
        let (mut stream, stream_state) = RecordingStream::with_incoming(vec![ClientMessage {
            message: Some(
                crate::proto::client::client_message::Message::WatchPlaybackSnapshot(
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                if message_sender
                    .sent_messages()
                    .iter()
                    .any(|message| matches!(message.message, Some(Message::PlaybackSnapshot(_))))
                {
                    break;
                }
                tokio::time::sleep(Duration::from_millis(10)).await;
    ...
    async fn test_watch_playback_state_with_current_version_skips_immediate_resend() {
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                let user_joined_sent = stream_state
                    ...
                    .iter()
                    .any(|message| matches!(message.message, Some(Message::UserJoined(_))));
                let watch_registered = handler.playback_watch_state.lock().await.state.enabled;
                if user_joined_sent && watch_registered {
                    break;
                }
                tokio::time::sleep(Duration::from_millis(10)).await;
            }
        ...
        .await
        .expect("run_after_join should emit UserJoined and register playback state watch");

        assert!(
            message_sender
                .sent_messages()
                .iter()
                .all(|message| { !matches!(message.message, Some(Message::PlaybackState(_))) }),
    ...
    async fn test_watch_playback_snapshot_with_current_version_and_matching_source_skips_immediate_resend(
        ...
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_pb_snap_same_src", message_sender.clone()).await;
        let StartTestFixture {
            handler,
            connection_service,
            event_service,
        ...
        assert!(
            message_sender
                .sent_messages()
                .iter()
                .all(|message| { !matches!(message.message, Some(Message::PlaybackSnapshot(_))) }),
    ...
    async fn test_watch_playback_snapshot_with_current_version_but_different_source_resends_immediately(
        ...
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_pb_snap_src_diff", message_sender.clone()).await;
        let StartTestFixture {
            handler,
            connection_service,
            event_service,
    ...
    async fn test_watched_playback_snapshot_refreshes_when_current_media_is_updated() {
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_pb_snap_md_upd", message_sender.clone()).await;
        let StartTestFixture {
            handler,
            connection_service,
            event_service,
    ...
    async fn test_watched_playback_snapshot_refreshes_when_current_playlist_is_updated() {
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_pb_snap_pl_upd", message_sender.clone()).await;
        let StartTestFixture {
            handler,
            connection_service,
            event_service,
    ...
    async fn test_watched_playback_snapshot_refreshes_when_target_changes_at_same_version() {
        ...
        .await
        .expect("target changes at the same DB version must refresh watched playback snapshots");
    ...
    async fn test_playback_snapshot_refresh_failure_disables_watch_without_closing_connection() {
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_pb_snap_refresh_fail", message_sender.clone())
                .await;
        let StartTestFixture {
            handler,
            connection_service,
    ...
    async fn test_watch_room_settings_without_version_sends_current_settings_immediately() {
        ...
        let (mut stream, stream_state) = RecordingStream::with_incoming(vec![ClientMessage {
            message: Some(
                crate::proto::client::client_message::Message::WatchRoomSettings(
    ...
    async fn test_watch_playlist_items_without_version_sends_snapshot_immediately() {
        ...
        let (mut stream, stream_state) = RecordingStream::with_incoming(vec![ClientMessage {
            message: Some(
                crate::proto::client::client_message::Message::WatchPlaylistItems(
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                if message_sender
                    .sent_messages()
                    .iter()
                    .any(|message| matches!(message.message, Some(Message::PlaylistItems(_))))
                {
                    break;
                }
                tokio::time::sleep(Duration::from_millis(10)).await;
    ...
    async fn test_watch_playlist_items_with_current_version_skips_immediate_resend() {
        let message_sender = RecordingMessageSender::new();
        let fixture = create_start_handler_fixture(
            "watch_playlist_items_same_version",
            message_sender.clone(),
        )
        .await;
        let StartTestFixture {
        ...
        assert!(
            message_sender
                .sent_messages()
                .iter()
                .all(|message| { !matches!(message.message, Some(Message::PlaylistItems(_))) }),
    ...
    async fn test_watched_playlist_items_receive_future_media_updates() {
        let message_sender = RecordingMessageSender::new();
        let fixture = create_start_handler_fixture(
            "watch_playlist_items_future_update",
            message_sender.clone(),
        )
        .await;
        let StartTestFixture {
    ...
    async fn test_watch_room_members_without_version_sends_snapshot_immediately() {
        let message_sender = RecordingMessageSender::new();
        let fixture =
            create_start_handler_fixture("watch_room_members_initial", message_sender.clone())
                .await;
        let StartTestFixture {
            handler,
            connection_service,
        ...
        let (mut stream, stream_state) = RecordingStream::with_incoming(vec![ClientMessage {
            message: Some(
                crate::proto::client::client_message::Message::WatchRoomMembers(
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                if message_sender
                    .sent_messages()
                    .iter()
                    .any(|message| matches!(message.message, Some(Message::RoomMembers(_))))
                {
                    break;
                }
                tokio::time::sleep(Duration::from_millis(10)).await;
    ...
    async fn test_watch_room_members_with_current_version_skips_immediate_resend() {
        ...
        assert!(
            message_sender
                .sent_messages()
                .iter()
                .all(|message| { !matches!(message.message, Some(Message::RoomMembers(_))) }),
    ...
    async fn test_watched_room_members_receive_future_permission_updates() {
        let message_sender = RecordingMessageSender::new();
        let fixture = create_start_handler_fixture(
            "watch_room_members_future_update",
            message_sender.clone(),
        )
        .await;
        let StartTestFixture {
    ...
    async fn test_watch_room_settings_with_current_version_skips_immediate_resend() {
        let message_sender = RecordingMessageSender::new();
        let fixture = create_start_handler_fixture(
            "watch_room_settings_same_version",
            message_sender.clone(),
        )
        .await;
        let StartTestFixture {
        ...
        assert!(
            message_sender
                .sent_messages()
                .iter()
                .all(|message| { !matches!(message.message, Some(Message::RoomSettings(_))) }),
    ...
    async fn test_watched_room_settings_receive_future_updates() {
        let message_sender = RecordingMessageSender::new();
        let fixture = create_start_handler_fixture(
            "watch_room_settings_future_update",
            message_sender.clone(),
        )
        .await;
        let StartTestFixture {
    ...
    fn test_chat_message_event_conversion() {
        ...
        match &msg.message {
            Some(Message::Chat(chat)) => {
                ...
                assert_eq!(chat.user_id, public_user_id());
    ...
    fn test_user_joined_event_conversion() {
        ...
        match &msgs[0].message {
            Some(Message::UserJoined(uj)) => {
                ...
                assert_eq!(member.user_id, public_user_id());
    ...
    fn test_user_left_event_conversion() {
        ...
        match &msgs[0].message {
            Some(Message::UserLeft(ul)) => {
                ...
                assert_eq!(ul.user_id, public_user_id());
            }
    ...
    fn test_media_removed_batch_event_conversion() {
        ...
        match &msgs[0].message {
            Some(Message::MediaRemovedBatch(batch)) => {
                ...
                assert_eq!(batch.removed_by_user_id, public_user_id());
            }
    ...
    fn test_playlist_reordered_event_conversion() {
        ...
        match &msgs[0].message {
            Some(Message::PlaylistReordered(reordered)) => {
                ...
                assert_eq!(reordered.reordered_by_user_id, public_user_id());
            }
    ...
    fn test_admin_events_return_empty() {
        ...
        assert!(
            cluster_event_to_server_messages(&event, "room_test", &public_id_codec()).is_empty()
        );
    ...
    #[tokio::test]
    async fn test_current_connection_matches_webrtc_recipient_requires_public_user_id() {
        let room_id = room_id();
        let user_id = user_id();
