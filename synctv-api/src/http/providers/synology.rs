        ...
        .execute_user_endpoint(
            ...
            move |auth| async move {
                ...
                if signed || public_owner != public_user {
                    let room_id = crate::synology_image_urls::verify_synology_image_access(
                    ...
                    .map_err(|error| match error {
                        crate::synology_image_urls::SynologyImageAccessError::Invalid => {
                            crate::impls::ApiError::Authentication(
                                "Invalid Synology image signature".to_string(),
                            )
                        }
                        crate::synology_image_urls::SynologyImageAccessError::WrongUser => {
...
) -> Result<crate::synology_image_urls::SynologyImageScope<'a>, crate::impls::ApiError> {
    let server_id = required(&query.server_id, "serverId")?;
    match query.kind.trim() {
        "file" => Ok(crate::synology_image_urls::SynologyImageScope::File {
            server_id,
            credential_owner_id,
            path: required(query.path.as_deref().unwrap_or_default(), "path")?,
            size: required(query.size.as_deref().unwrap_or("medium"), "size")?,
        }),
        "poster" => Ok(crate::synology_image_urls::SynologyImageScope::Poster {
            server_id,
            credential_owner_id,
            item_id: query.item_id.filter(|value| *value > 0).ok_or_else(|| {
                crate::impls::ApiError::InvalidInput("itemId must be greater than zero".to_string())
            })?,
...
fn required<'a>(value: &'a str, field: &str) -> Result<&'a str, crate::impls::ApiError> {
