...
<Steps>
1. Call `GetAuthorizationUrl` to get the provider authorization URL and state.
2. Redirect the user to the provider.
3. The provider redirects back to the client URL with `code` and `state`.
4. Call `ExchangeAuthorizationCode`.
5. The server validates state, exchanges the code for user info, and returns SyncTV tokens.
</Steps>
