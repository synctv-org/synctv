...
如果你按 [快速开始](../../guides/quick-start/) 使用生产 Compose，通常不需要先写 `synctv.yaml`。默认入口是 `.env.postgres` 和 `.env.synctv`。
...
# init-compose-env.sh 会生成这些长期 secret。请备份并复用同一组文件。
SYNCTV_JWT_SECRET=...
...
需要裸机、Kubernetes、自定义 secret file 或完整 YAML 时，再看下面的 YAML 模板。
...
<Aside type="note">
这份 YAML 骨架更适合裸机、Kubernetes 或自定义 secret file 部署。生产 Compose 用户优先维护 `.env.postgres` 和 `.env.synctv`。
</Aside>
