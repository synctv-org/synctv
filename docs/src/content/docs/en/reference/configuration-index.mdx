...
| `http_rate_limits` | object | see below | [Rate Limits and Connection Limits](../../configuration/rate-limits/) |
| `grpc_rate_limits` | object | see below | [Rate Limits and Connection Limits](../../configuration/rate-limits/) |
...
| `livestream.hls_storage_backend` | `memory`, `file`, `shared_file`, or `oss` | `memory` | HLS segment storage backend |
| `livestream.hls_storage_path` | path | `""` | HLS file path for the `file` / `shared_file` backend |
...
| `file_storage.chat_images_backend` | backend name or empty | `""` | Backend for chat images; empty inherits `default_backend` |
| `file_storage.user_avatars_backend` | backend name or empty | `""` | Backend for user avatars; empty inherits `default_backend` |
| `file_storage.video_covers_backend` | backend name or empty | `""` | Backend for video covers; empty inherits `default_backend` |
...
| `file_storage.backends.<name>.database.compression` | `none`, `lz4`, or `zstd` | `zstd` | Compression algorithm used by `type: "database"` backends for bytes stored in PostgreSQL `file_blobs` |
| `file_storage.backends.<name>.s3.*` | S3 config | default S3 fields | S3-compatible settings for backends with `type: "s3"` |

Clients provide `checksum_sha256` in upload-session requests. The selected backend stores content-addressed objects by `(storage_backend, checksum_sha256, size_bytes)`, so duplicate files return `upload_required=false` with an ownership proof challenge. Reads and cleanup use each object's stored `storage_backend`, allowing product data to mix backends across configuration changes.
...
| `cluster.secret_file` | file path or empty | `""` | File containing the internal cluster gRPC authentication secret |
...
| `cache.permission_cache_capacity` | integer | `20000` | Permission cache capacity |
...
| `grpc_rate_limits.websocket_max_requests` | integer | `10` | Maximum gRPC MessageStream connection attempts per window |
| `grpc_rate_limits.websocket_window_seconds` | seconds | `60` | gRPC MessageStream connection-attempt rate-limit window |
