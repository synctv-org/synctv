...
pub const CLUSTER_OUTBOX_CHANNEL: &str = "cluster_outbox_new";
const DEFAULT_MAX_ATTEMPTS: i32 = 12;

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ClusterOutboxStatus {
    Pending,
    Processing,
    Sent,
    Dead,
    ...
    fn parse(value: &str) -> Result<Self> {
        match value {
            ...
            other => Err(Error::Internal(format!(
                "Unknown cluster outbox status: {other}"
            ))),
        }
    }
}
...
pub struct NewClusterOutboxEvent {
    pub id: String,
...
pub struct ClusterOutboxEvent {
    ...
    pub aggregate_version: Option<i64>,
    pub payload: Value,
    pub status: ClusterOutboxStatus,
    pub attempts: i32,
...
impl ClusterOutboxEvent {
    fn from_row(row: &sqlx::postgres::PgRow) -> Result<Self> {
        ...
        Ok(Self {
            ...
            status: ClusterOutboxStatus::parse(&status)?,
...
#[derive(Clone)]
pub struct ClusterOutboxRepository {
    pool: PgPool,
}

impl ClusterOutboxRepository {
    #[must_use]
    ...
    pub async fn insert(&self, event: &NewClusterOutboxEvent) -> Result<()> {
        self.insert_with_executor(event, &self.pool).await
    }

    pub async fn insert_with_executor<'e, E>(
        &self,
        event: &NewClusterOutboxEvent,
        executor: E,
    ) -> Result<()>
    where
        E: sqlx::PgExecutor<'e>,
    ...
    pub async fn claim_batch(
        ...
        rows.iter().map(ClusterOutboxEvent::from_row).collect()
    }
    ...
    pub async fn mark_failed(&self, id: &str, attempts: i32, error: &str) -> Result<()> {
        ...
        let status = if next_attempt >= DEFAULT_MAX_ATTEMPTS {
            ClusterOutboxStatus::Dead
        } else {
            ClusterOutboxStatus::Pending
        };
