...
Docker Compose loads `.env.postgres` and `.env.synctv` through `env_file`: PostgreSQL reads only database variables, and `synctv` reads only application settings. Back up both files and reuse them across restarts, new shells, host reboots, and upgrades.
