...
use synctv_core::repository::query_builder::trusted_dynamic_sql;
use synctv_core::resilience::timeout::DB_QUERY_TIMEOUT;
...
impl std::fmt::Debug for DatabasePoolOptions {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DatabasePoolOptions")
...
pub struct DatabaseInitOptions {
    pub database: DatabasePoolOptions,
    pub logging: LoggingOptions,
}

impl DatabaseInitOptions {
    #[must_use]
...
pub async fn init_database(options: &DatabaseInitOptions) -> Result<DatabaseInit> {
    init_database_with_cancel(options, None).await
}
...
pub async fn init_database_with_cancel(
    ...
    init_database_inner(options, cancel, false).await
}
...
pub async fn init_database_with_read_pool_and_cancel(
    ...
    init_database_inner(options, cancel, true).await
}
...
async fn init_database_inner(
    ...
    let pg_client_min_messages = pg_client_min_messages_for_level(
        synctv_core::logging::effective_log_level(&options.logging)?,
    );
    ...
    .await?;
    let read_pool = match (include_read_pool, options.database_read_url()) {
    ...
    let max_connections_per_pool = options.database.max_connections;
    let max_connections_total = max_connections_per_pool.saturating_mul(pool_count);
    synctv_core::metrics::database::DB_POOL_SIZE_MAX.set(i64::from(max_connections_total));
    ...
    let metrics_task = cancel.map(|token| {
        ...
        synctv_core::spawn::spawn_monitored("db_pool_metrics", async move {
...
async fn connect_database_pool(
    ...
    let pool: PgPool = PgPoolOptions::new()
        .max_connections(options.database.max_connections)
        .min_connections(options.database.min_connections)
        .acquire_timeout(Duration::from_secs(
            options.database.connect_timeout_seconds,
        ))
        .idle_timeout(Duration::from_secs(options.database.idle_timeout_seconds))
        .max_lifetime(Duration::from_secs(options.database.max_lifetime_seconds))
...
fn record_database_pool_metrics(pools: &DatabasePools, max_connections_per_pool: u32) {
    ...
    synctv_core::metrics::database::DB_CONNECTIONS_ACTIVE
        .set(i64::from(primary.active.saturating_add(read.active)));
    synctv_core::metrics::database::DB_CONNECTIONS_IDLE
...
fn record_pool_utilization(role: &'static str, pool: &PgPool, max_connections_per_pool: u32) {
    ...
    if max_connections_per_pool > 0 {
        synctv_core::metrics::database::DB_POOL_UTILIZATION
...
mod tests {
    use super::*;
    use synctv_core::service::acquire_unbounded_ddl_connection;
    ...
    async fn ddl_connection_statement_timeout_is_reset_when_returned_to_pool() {
        ...
        let options = DatabaseInitOptions {
            database: DatabasePoolOptions {
                url: database_url,
                max_connections: 5,
                min_connections: 1,
        ...
        let pool = init_database_with_cancel(&options, None)
            .await
    ...
    async fn init_database_without_cancel_does_not_spawn_unmanaged_metrics_task() {
        ...
        let db_init = init_database(&options)
            .await
    ...
    async fn init_database_with_read_pool_requires_configured_read_pool() {
        ...
        let error = init_database_with_read_pool_and_cancel(&options, None)
            .await
    ...
    async fn init_database_with_cancel_returns_stoppable_metrics_task() {
        ...
        let db_init = init_database_with_cancel(&options, Some(cancel.clone()))
            .await
