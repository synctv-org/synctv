...
In multi-replica deployments, cleanup tasks are leader-gated to avoid duplicate cleanup work. Database maintenance runs at startup and periodically to clean email tokens, notifications, expired provider credentials, old chat messages, and file objects that were uploaded but never attached to a product row.

<Aside type="note">
Runtime settings are database-backed hot-reload settings. They are appropriate for `chat.message_retention_days` and `chat.max_messages_per_room`. They do not replace startup configuration and must not store secrets.
</Aside>
