...
import { englishComposeDeploymentDocsUrl } from '../../../../lib/project';
...
<Steps>
1. Generate persistent env files. Before running this, get `docker-compose.yml`, env examples, and the init script from <a href={englishComposeDeploymentDocsUrl}>Docker Compose Deployment</a>:
...
Clone the source repository first, then run commands from the repository root. Development Compose needs the Dockerfile and source tree.
...
First get `docker-compose.yml`, `.env.postgres.example`, `.env.synctv.example`, and `scripts/init-compose-env.sh` from [Docker Compose Deployment](../../deployment/docker-compose/#get-the-compose-files).

Production Compose intentionally refuses to start without explicit secrets. Generate and keep persistent `.env.postgres` and `.env.synctv` files in the Compose file directory:
