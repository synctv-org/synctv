...
pub(crate) const PREVIEW_ROUTE: &str = "/api/providers/nextcloud/preview";
const SIGNATURE_PROVIDER: &str = "nextcloud-preview";

#[derive(Clone, Copy)]
pub(crate) struct NextcloudPreviewScope<'a> {
    pub server_id: &'a str,
...
pub(crate) enum NextcloudPreviewAccessError {
    Invalid,
    WrongUser,
}
...
pub(crate) fn nextcloud_preview_url(
    server_id: &str,
...
pub(crate) fn sign_nextcloud_preview_url(
    url: &str,
    room_id: &str,
    user_id: &str,
...
pub(crate) fn verify_nextcloud_preview_access(
    signing_key: &ProxySigningKey,
    auth_user_id: &str,
    raw_query: &str,
