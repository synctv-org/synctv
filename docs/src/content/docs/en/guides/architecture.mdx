...
<Diagram
  ...
  caption="Recommended first production model: one SyncTV service entrypoint, PostgreSQL, and Redis. Redis is optional for simple single-node deployments, but recommended in production."
/>
...
- HLS cross-node reads currently use publisher-node proxying. Shared filesystem storage or OSS improves storage, recovery, and cleanup boundaries, but capacity planning should still include the publisher-node proxy request path.
...
<Diagram
  ...
  caption="Kubernetes multi-replica deployments should split HTTP and gRPC Services/Ingresses and connect every pod to the same PostgreSQL and Redis backends. HLS cross-node reads currently use publisher-node proxying; shared filesystem storage or OSS provides a more stable storage and recovery boundary."
/>
