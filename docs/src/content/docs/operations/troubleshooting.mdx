...
| `required variable ... is missing` | 生产 Compose 缺 `.env.postgres` 或 `.env.synctv` | 运行 `./scripts/init-compose-env.sh` |
...
| 媒体 | Provider header、Range、proxy、slice cache、HLS backend | [播放与代理模型](../../use/playback-and-proxy/) |
...
- 服务日志中的第一条错误。
- HTTP 错误响应里的 `request_id`、`status` 和 `code`。
