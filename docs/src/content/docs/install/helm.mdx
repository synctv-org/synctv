...
如果同时启用 `networkPolicy.enabled=true`，external PostgreSQL/Redis 的 egress 不再匹配 Chart 内部 Pod label；默认允许对应端口访问 `0.0.0.0/0`。生产环境建议通过 `networkPolicy.externalPostgresqlCIDRs` 和 `networkPolicy.externalRedisCIDRs` 收紧到实际数据库网段。
