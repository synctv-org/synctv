...
清理任务在多副本部署中由 leader-gated 后台任务执行，避免多个副本重复清理。数据库维护任务会在启动时和周期性运行时清理邮箱 token、通知、过期 Provider 凭据、旧聊天消息，以及已上传但未被业务行引用的文件对象。

<Aside type="note">
Runtime settings 是数据库热更新设置，适合调整 `chat.message_retention_days` 和 `chat.max_messages_per_room`。它不替代启动配置，也不用于保存 secret。
</Aside>
